...
(** ** The type of sets i.e. of types of h-level 2 in [ Type ] *)

Definition hSet:= total2 (fun X : Type => isaset X) .
Definition hSetpair X i := tpair isaset X i : hSet.
Definition pr1hSet:= @pr1 Type (fun X : Type => isaset X) : hSet -> Type.
Coercion pr1hSet: hSet >-> Type .
...
Lemma isaset_forall_hSet (X:Type) (Y:X->hSet) : isaset (∀ x, Y x).
Proof. intros. apply impred_isaset. intro x. apply setproperty. Defined.

Definition forall_hSet {X:Type} (Y:X->hSet) : hSet := hSetpair (∀ x, Y x) (isaset_forall_hSet X Y).
...
Definition ischoicebase_uu1 ( X : Type ) := ∀ P : X -> Type , ( ∀ x : X , ishinh ( P x ) ) -> ishinh ( ∀ x : X , P x ) .

Lemma isapropischoicebase ( X : Type ) : isaprop ( ischoicebase_uu1 X ) .  (** Uses RR1 *)
Proof .  intro . apply impred . intro P .  apply impred . intro fs . apply ( pr2 ( ishinh _ ) ) .  Defined .

Definition ischoicebase ( X : Type ) : hProp := hProppair _ ( isapropischoicebase X ) .


Lemma ischoicebaseweqf { X Y : Type } ( w : X ≃ Y ) ( is : ischoicebase X ) : ischoicebase Y .
...
Lemma ischoicebaseweqb { X Y : Type } ( w : X ≃ Y ) ( is : ischoicebase Y ) : ischoicebase X .
...
Lemma ischoicebasecontr { X : Type } ( is : iscontr X ) : ischoicebase X .
...
Lemma ischoicebaseempty2 { X : Type } ( is : ¬ X ) : ischoicebase X .
Proof . intros . apply ( ischoicebaseweqb ( weqtoempty is ) ischoicebaseempty ) . Defined .

Lemma ischoicebasecoprod { X Y : Type } ( isx : ischoicebase X ) ( isy : ischoicebase Y ) :  ischoicebase ( coprod X Y ) .
...
Definition hsubtypes ( X : Type ) :=  X -> hProp .
Identity Coercion id_hsubtypes :  hsubtypes >-> Funclass .
Definition carrier { X : Type } ( A : hsubtypes X ) := total2 A.
Coercion carrier : hsubtypes >-> Sortclass.
Definition carrierpair { X : Type } ( A : hsubtypes X ) := tpair A.
Definition pr1carrier { X:Type } ( A : hsubtypes X ) := @pr1 _ _  : carrier A -> X .

Lemma isinclpr1carrier { X : Type } ( A : hsubtypes X ) : isincl ( @pr1carrier X A ) .
Proof . intros . apply ( isinclpr1 A ( fun x : _ => pr2 ( A x ) ) ) . Defined .

Lemma isasethsubtypes (X : Type) : isaset (hsubtypes X).
Proof.
intro X.
...
Definition totalsubtype ( X : Type ) : hsubtypes X := fun x => htrue .

Definition weqtotalsubtype ( X : Type ) : totalsubtype X ≃ X .
...
Definition subtypesdirprod { X Y : Type } ( A : hsubtypes X ) ( B : hsubtypes Y ) : hsubtypes ( dirprod X Y ) := fun xy : _ => hconj ( A ( pr1 xy ) ) ( B ( pr2 xy ) ) .

Definition fromdsubtypesdirprodcarrier { X Y : Type } ( A : hsubtypes X ) ( B : hsubtypes Y ) ( xyis : subtypesdirprod A B ) : dirprod A B .
...
Definition tosubtypesdirprodcarrier { X Y : Type } ( A : hsubtypes X ) ( B : hsubtypes Y ) ( xisyis : dirprod A B ) : subtypesdirprod A B .
...
Lemma weqsubtypesdirprod { X Y : Type } ( A : hsubtypes X ) ( B : hsubtypes Y ) : subtypesdirprod A B ≃ A × B .
...
Lemma ishinhsubtypesdirprod  { X Y : Type } ( A : hsubtypes X ) ( B : hsubtypes Y ) ( isa : ishinh A ) ( isb : ishinh B ) : ishinh ( subtypesdirprod A B ) .
...
Lemma isapropsubtype { X : Type } ( A : hsubtypes X ) ( is : ∀ ( x1 x2 : X ) , A x1 -> A x2 -> x1 = x2 ) : isaprop ( carrier A ) .
...
Definition squash_pairs_to_set {Y} (F:Y->Type) :
...
Definition hrel ( X : Type ) := X -> X -> hProp.
Identity Coercion idhrel : hrel >-> Funclass .

Definition brel ( X : Type ) := X -> X -> bool .
...
Definition istrans { X : Type } ( R : hrel X ) := ∀ ( x1 x2 x3 : X ), R x1 x2 -> R x2 x3 -> R x1 x3.

Definition isrefl { X : Type } ( R : hrel X ) := ∀ x : X , R x x.

Definition issymm { X : Type } ( R : hrel X ) := ∀ ( x1 x2 : X ), R x1 x2 -> R x2 x1 .

Definition ispreorder { X : Type } ( R : hrel X ) := istrans R × isrefl R .

Definition iseqrel { X : Type } ( R : hrel X ) := ispreorder R × issymm R .
Definition iseqrelconstr { X : Type } { R : hrel X } ( trans0 : istrans R ) ( refl0 : isrefl R ) ( symm0 : issymm R ) : iseqrel R := dirprodpair ( dirprodpair trans0 refl0 ) symm0 .

Definition isirrefl { X : Type } ( R : hrel X ) := ∀ x : X , ¬ R x x .

Definition isasymm { X : Type } ( R : hrel X ) := ∀ ( x1 x2 : X ), R x1 x2 -> R x2 x1 -> empty .

Definition iscoasymm { X : Type } ( R : hrel X ) := ∀ x1 x2 , ¬ R x1 x2 -> R x2 x1 .

Definition istotal { X : Type } ( R : hrel X ) := ∀ x1 x2 , R x1 x2 ∨ R x2 x1 .

Definition isdectotal { X : Type } ( R : hrel X ) := ∀ x1 x2 , R x1 x2 ⨿ R x2 x1 .

Definition iscotrans { X : Type } ( R : hrel X ) := ∀ x1 x2 x3 , R x1 x3 -> R x1 x2 ∨ R x2 x3 .

Definition isdeccotrans { X : Type } ( R : hrel X ) := ∀ x1 x2 x3 , R x1 x3 -> R x1 x2 ⨿ R x2 x3 .

Definition isdecrel { X : Type } ( R : hrel X ) := ∀ x1 x2 , R x1 x2 ⨿ ¬ R x1 x2 .

Definition isnegrel { X : Type } ( R : hrel X ) := ∀ x1 x2 , ¬ ¬ R x1 x2 -> R x1 x2 .
...
Definition isantisymm { X : Type } ( R : hrel X ) := ∀ ( x1 x2 : X ), R x1 x2 -> R x2 x1 -> x1 = x2 .

Definition isPartialOrder { X : Type } ( R : hrel X ) := ispreorder R × isantisymm R.

Ltac unwrap_isPartialOrder i :=
  induction i as [transrefl antisymm]; induction transrefl as [trans refl].

Definition isantisymmneg { X : Type } ( R : hrel X ) := ∀ ( x1 x2 : X ), ¬ R x1 x2 -> ¬ R x2 x1 -> x1 = x2 .

Definition iscoantisymm { X : Type } ( R : hrel X ) := ∀ x1 x2 , ¬ R x1 x2 -> R x2 x1 ⨿ (x1 = x2) .
...
Definition neqchoice { X : Type } ( R : hrel X ) := ∀ x1 x2 , x1 != x2 -> R x1 x2 ⨿ R x2 x1 .
...
Lemma istransandirrefltoasymm { X : Type } { R : hrel X } ( is1 : istrans R ) ( is2 : isirrefl R ) : isasymm R .
Proof . intros .  intros a b rab rba . apply ( is2 _ ( is1 _ _ _ rab rba ) ) .  Defined .

Lemma istotaltoiscoasymm { X : Type } { R : hrel X } ( is : istotal R ) : iscoasymm R .
Proof . intros .  intros x1 x2 . apply ( hdisjtoimpl ( is _ _ ) ) . Defined .

Lemma isdecreltoisnegrel { X : Type } { R : hrel X } ( is : isdecrel R ) : isnegrel R .
Proof . intros .  intros x1 x2 .  destruct ( is x1 x2 ) as [ r | nr ] . intro . apply r . intro nnr . destruct ( nnr nr ) .  Defined .

Lemma isantisymmnegtoiscoantisymm { X : Type } { R : hrel X } ( isdr : isdecrel R ) ( isr : isantisymmneg R ) : iscoantisymm R .
Proof . intros . intros x1 x2 nrx12 . destruct ( isdr x2 x1 ) as [ r | nr ] . apply ( ii1 r ) .  apply ii2 . apply ( isr _ _ nrx12 nr ) .  Defined .

Lemma rtoneq { X : Type } { R : hrel X } ( is : isirrefl R ) { a b : X } ( r : R a b ) : a != b .
Proof . intros . intro e . rewrite e in r . apply ( is b r ) . Defined .


(** *** Standard properties of relations and logical equivalences *)

Definition hrellogeq { X : Type } ( L R : hrel X ) := ∀ x1 x2 , ( L x1 x2 <-> R x1 x2 ) .

Definition istranslogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : istrans L ) : istrans R .
Proof . intros . intros x1 x2 x3 r12 r23 .   apply ( ( pr1 ( lg _ _ ) ) ( isl _ _ _ ( ( pr2 ( lg _ _ ) ) r12 ) ( ( pr2 ( lg _ _ ) ) r23 ) ) ) . Defined .

Definition isrefllogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : isrefl L ) : isrefl R .
Proof . intros . intro x . apply ( pr1 ( lg _ _ ) ( isl x ) ) .  Defined .

Definition issymmlogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : issymm L ) : issymm R .
Proof . intros . intros x1 x2 r12 . apply ( pr1 ( lg _ _ ) ( isl _ _ ( pr2 ( lg _ _ ) r12 ) ) ) . Defined .

Definition ispologeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : ispreorder L ) : ispreorder R .
Proof . intros . apply ( dirprodpair ( istranslogeqf lg ( pr1 isl ) ) ( isrefllogeqf lg ( pr2 isl ) ) ) . Defined .

Definition iseqrellogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : iseqrel L ) : iseqrel R .
Proof . intros . apply ( dirprodpair ( ispologeqf lg ( pr1 isl ) ) ( issymmlogeqf lg ( pr2 isl ) ) ) . Defined .

Definition isirrefllogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : isirrefl L ) : isirrefl R .
Proof . intros . intros x r . apply ( isl _ ( pr2 ( lg x x ) r ) ) . Defined .

Definition isasymmlogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : isasymm L ) : isasymm R .
Proof . intros . intros x1 x2 r12 r21 . apply ( isl _ _ ( pr2 ( lg _ _ ) r12 ) ( pr2 ( lg _ _ ) r21 ) )   . Defined .

Definition iscoasymmlogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : iscoasymm L ) : iscoasymm R .
Proof . intros . intros x1 x2 r12 . apply ( ( pr1 ( lg _ _ ) ) ( isl _ _ ( negf ( pr1 ( lg _ _ ) ) r12 ) ) ) . Defined .

Definition istotallogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : istotal L ) : istotal R .
...
Definition iscotranslogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : iscotrans L ) : iscotrans R .
...
Definition isdecrellogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : isdecrel L ) : isdecrel R .
Proof . intros . intros x1 x2 . destruct ( isl x1 x2 ) as [ l | nl ] . apply ( ii1 ( pr1 ( lg _ _ ) l ) ) . apply ( ii2 ( negf ( pr2 ( lg _ _ ) ) nl ) ) . Defined .

Definition isnegrellogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : isnegrel L ) : isnegrel R .
Proof . intros . intros x1 x2 nnr . apply ( ( pr1 ( lg _ _ ) ) ( isl _ _ ( negf ( negf ( pr2 ( lg _ _ ) ) ) nnr ) ) ) . Defined .

Definition isantisymmlogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : isantisymm L ) : isantisymm R .
Proof . intros . intros x1 x2 r12 r21 . apply ( isl _ _ ( pr2 ( lg _ _ ) r12 ) ( pr2 ( lg _ _ ) r21 ) )   . Defined .

Definition isantisymmneglogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : isantisymmneg L ) : isantisymmneg R .
Proof . intros . intros x1 x2 nr12 nr21 . apply ( isl _ _ ( negf ( pr1 ( lg _ _ ) ) nr12 ) ( negf ( pr1 ( lg _ _ ) ) nr21 ) )   . Defined .

Definition iscoantisymmlogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : iscoantisymm L ) : iscoantisymm R .
...
Definition neqchoicelogeqf { X : Type } { L R : hrel X } ( lg : ∀ x1 x2 , L x1 x2 <-> R x1 x2 ) ( isl : neqchoice L ) : neqchoice R .
...
(* preoderings *)
Definition po (X:Type) := Σ R:hrel X, ispreorder R.
Definition popair { X : Type } ( R : hrel X ) ( is : ispreorder R ) : po X := tpair ispreorder R is .
Definition carrierofpo ( X : Type ) :  po X  -> ( X -> X -> hProp ) :=  @pr1 _ ispreorder .
...
Definition eqrel ( X : Type ) := total2 ( fun R : hrel X => iseqrel R ) .
Definition eqrelpair { X : Type } ( R : hrel X ) ( is : iseqrel R ) : eqrel X := tpair ( fun R : hrel X => iseqrel R ) R is .
Definition eqrelconstr { X : Type } ( R : hrel X ) ( is1 : istrans R ) ( is2 : isrefl R ) ( is3 : issymm R ) : eqrel X := eqrelpair R ( dirprodpair ( dirprodpair is1 is2 ) is3 ) .
Definition pr1eqrel ( X : Type ) : eqrel X -> ( X -> ( X -> hProp ) ) := @pr1 _ _ .
Coercion pr1eqrel : eqrel >-> Funclass .

Definition eqreltrans { X : Type } ( R : eqrel X ) : istrans R := pr1 ( pr1 ( pr2 R ) ) .
Definition eqrelrefl { X : Type } ( R : eqrel X ) : isrefl R := pr2 ( pr1 ( pr2 R ) ) .
Definition eqrelsymm { X : Type } ( R : eqrel X ) : issymm R := pr2 ( pr2 R )  .



(** *** Direct product of two relations *)

Definition hreldirprod { X Y : Type } ( RX : hrel X ) ( RY : hrel Y ) : hrel ( dirprod X Y ) := fun xy xy' : dirprod X Y => hconj ( RX ( pr1 xy ) ( pr1 xy' ) ) ( RY ( pr2 xy ) ( pr2 xy' ) ) .

Definition istransdirprod { X Y : Type } ( RX : hrel X ) ( RY : hrel Y ) ( isx : istrans RX ) ( isy : istrans RY ) : istrans ( hreldirprod RX RY ) := fun xy1 xy2 xy3 : _ => fun is12 : _  => fun is23 : _ => dirprodpair ( isx _ _ _ ( pr1 is12 ) ( pr1 is23 ) ) ( isy _ _ _ ( pr2 is12 ) ( pr2 is23 ) ) .

Definition isrefldirprod { X Y : Type } ( RX : hrel X ) ( RY : hrel Y ) ( isx : isrefl RX ) ( isy : isrefl RY ) : isrefl ( hreldirprod RX RY ) := fun xy : _ => dirprodpair ( isx _ ) ( isy _ ) .

Definition   issymmdirprod { X Y : Type } ( RX : hrel X ) ( RY : hrel Y ) ( isx : issymm RX ) ( isy : issymm RY ) : issymm ( hreldirprod RX RY ) :=  fun xy1 xy2 : _ => fun is12 : _ => dirprodpair ( isx _ _ ( pr1 is12 ) ) ( isy _ _ ( pr2 is12 ) ) .

Definition eqreldirprod { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) : eqrel ( dirprod X Y ) := eqrelconstr ( hreldirprod RX RY ) ( istransdirprod _ _ ( eqreltrans RX ) ( eqreltrans RY ) ) ( isrefldirprod  _ _ ( eqrelrefl RX ) ( eqrelrefl RY ) ) ( issymmdirprod  _ _ ( eqrelsymm RX ) ( eqrelsymm RY ) ) .


(** *** Negation of a relation and its properties *)

Definition negrel { X : Type } ( R : hrel X ) : hrel X
  := λ x x', hProppair (¬ R x x') (isapropneg _) . (* uses [funextempty] *)

Lemma istransnegrel { X : Type } ( R : hrel X  ) ( isr : iscotrans R ) : istrans ( negrel R ) .
...
Lemma isasymmnegrel { X : Type } ( R : hrel X  ) ( isr : iscoasymm R ) : isasymm ( negrel R ) .
Proof . intros . intros x1 x2 r12 r21 . apply ( r21 ( isr _ _ r12 ) ) .   Defined .

Lemma iscoasymmgenrel { X : Type } ( R : hrel X  ) ( isr : isasymm R ) : iscoasymm ( negrel R ) .
Proof . intros . intros x1 x2 nr12 . apply ( negf ( isr _ _ ) nr12 ) .  Defined .

Lemma isdecnegrel { X : Type } ( R : hrel X  ) ( isr : isdecrel R ) : isdecrel ( negrel R ) .
(* uses [funextempty] *)
Proof . intros . intros x1 x2 . destruct ( isr x1 x2 ) as [ r | nr ] . apply ii2 .   apply ( todneg _ r ) .  apply ( ii1 nr ) . Defined .

Lemma isnegnegrel { X : Type } ( R : hrel X ) : isnegrel ( negrel R ) .
Proof . intros .   intros x1 x2 .  apply ( negf ( todneg ( R x1 x2 ) ) ) . Defined .

Lemma isantisymmnegrel { X : Type } ( R : hrel X  ) ( isr : isantisymmneg R ) : isantisymm ( negrel R ) .
Proof . intros . apply isr .  Defined .

(** *** Boolean representation of decidable equality *)

Definition eqh { X : Type } ( is : isdeceq X ) : hrel X := fun x x' => hProppair ( booleq is x x' = true ) ( isasetbool ( booleq is x x' ) true ) .

Definition neqh { X : Type } ( is : isdeceq X ) : hrel X := fun x x' =>  hProppair ( booleq is x x' = false ) ( isasetbool ( booleq is x x' ) false ) .

Lemma isrefleqh { X : Type } ( is : isdeceq X ) : isrefl ( eqh is ) .
Proof . intros .  unfold eqh .  unfold booleq . intro x .  destruct ( is x x ) as [ e | ne ] . simpl .  apply idpath .  destruct ( ne ( idpath x ) ) .  Defined .

Definition weqeqh { X : Type } ( is : isdeceq X ) ( x x' : X ) : ( x = x' ) ≃ ( eqh is x x' ) .
...
Definition weqneqh { X : Type } ( is : isdeceq X ) ( x x' : X ) : ( x != x' ) ≃ ( neqh is x x' ) .
...
Definition decrel ( X : Type ) := total2 ( fun R : hrel X => isdecrel R ) .
Definition pr1decrel ( X : Type ) : decrel X -> hrel X := @pr1 _ _ .
Definition decrelpair { X : Type } { R : hrel X } ( is : isdecrel R ) : decrel X := tpair _ R is .
Coercion pr1decrel : decrel >-> hrel .

Definition decreltobrel { X : Type } ( R : decrel X ) : brel X .
Proof . intros . intros x x' . destruct ( ( pr2 R ) x x' ) . apply true . apply false . Defined .

Definition breltodecrel { X : Type } ( B : brel X ) : decrel X := @decrelpair _ ( fun x x' => hProppair ( paths ( B x x' ) true ) ( isasetbool _ _ ) ) ( fun x x' => ( isdeceqbool _ _ ) ) .
...
Definition pathstor { X : Type } ( R : decrel X ) ( x x' : X ) ( e : decreltobrel R x x' = true ) : R x x' .
Proof . unfold decreltobrel . intros .  destruct ( pr2 R x x' ) as [ e' | ne ]  .  apply e' . destruct ( nopathsfalsetotrue e ) . Defined .

Definition rtopaths  { X : Type } ( R : decrel X ) ( x x' : X ) ( r : R x x' ) : decreltobrel R x x' = true  .
Proof . unfold decreltobrel .  intros . destruct ( ( pr2 R ) x x' ) as [ r' | nr ] . apply idpath .  destruct ( nr r ) . Defined .

Definition pathstonegr { X : Type } ( R : decrel X ) ( x x' : X ) ( e : decreltobrel R x x' = false ) : neg ( R x x' ) .
Proof . unfold decreltobrel . intros .  destruct ( pr2 R x x' ) as [ e' | ne ] .  destruct ( nopathstruetofalse e ) . apply ne .  Defined .

Definition negrtopaths { X : Type } ( R : decrel X ) ( x x' : X ) ( nr : neg ( R x x' ) ) : decreltobrel R x x' = false .
...
Definition pathstor_comp { X : Type } ( R : decrel X ) ( x x' : X ) ( e : paths ( decreltobrel R x x' ) true ) : R x x' .
Proof . unfold decreltobrel . intros .  destruct ( pr2 R x x' ) as [ e' | ne ]  .  apply e' . destruct ( nopathsfalsetotrue e ) . Defined .
...
Definition ctlong { X : Type } ( R : hrel X ) ( is : isdecrel R ) ( x x' : X ) ( e : decreltobrel (decrelpair is ) x x' = true ) : R x x' .
...
Definition resrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) : hrel P := fun p1 p2 => L ( pr1 p1 ) ( pr1 p2 ) .

Definition istransresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : istrans L ) : istrans ( resrel L P ) .
Proof . intros . intros x1 x2 x3 r12 r23 . apply ( isl _ ( pr1 x2 ) _ r12 r23 ) . Defined .

Definition isreflresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X )  ( isl : isrefl L ) : isrefl ( resrel L P ) .
Proof . intros . intro x . apply isl . Defined .

Definition issymmresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : issymm L ) : issymm ( resrel L P ) .
Proof . intros . intros x1 x2 r12 . apply isl . apply r12 .  Defined .

Definition isporesrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : ispreorder L ) : ispreorder ( resrel L P ) .
Proof . intros . apply ( dirprodpair ( istransresrel L P ( pr1 isl ) ) ( isreflresrel L P ( pr2 isl ) ) ) . Defined .

Definition iseqrelresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : iseqrel L ) : iseqrel ( resrel L P ) .
Proof . intros . apply ( dirprodpair ( isporesrel L P ( pr1 isl ) ) ( issymmresrel L P ( pr2 isl ) ) ) . Defined .

Definition isirreflresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : isirrefl L ) : isirrefl ( resrel L P ) .
Proof . intros . intros x r . apply ( isl _ r ) . Defined .

Definition isasymmresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : isasymm L ) : isasymm ( resrel L P ) .
Proof . intros . intros x1 x2 r12 r21 . apply ( isl _ _ r12 r21 ) .  Defined .

Definition iscoasymmresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : iscoasymm L ) : iscoasymm ( resrel L P ) .
Proof . intros . intros x1 x2 r12 . apply ( isl _ _ r12 ) . Defined .

Definition istotalresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : istotal L ) : istotal ( resrel L P ) .
Proof . intros . intros x1 x2 . apply isl . Defined .

Definition iscotransresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : iscotrans L ) : iscotrans ( resrel L P ) .
Proof . intros . intros x1 x2 x3 r13 . apply ( isl _ _ _ r13 ) .  Defined .

Definition isdecrelresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : isdecrel L ) : isdecrel ( resrel L P ) .
Proof . intros . intros x1 x2 . apply isl . Defined .

Definition isnegrelresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : isnegrel L ) : isnegrel ( resrel L P ) .
Proof . intros . intros x1 x2 nnr . apply ( isl _ _ nnr ) . Defined .

Definition isantisymmresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : isantisymm L ) : isantisymm ( resrel L P ) .
Proof . intros . intros x1 x2 r12 r21 . apply ( invmaponpathsincl _ ( isinclpr1carrier _ ) _ _ ( isl _ _ r12 r21  ) ) . Defined .

Definition isantisymmnegresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : isantisymmneg L ) : isantisymmneg ( resrel L P ) .
Proof . intros . intros x1 x2 nr12 nr21 . apply (  invmaponpathsincl _ ( isinclpr1carrier _ ) _ _ ( isl _ _ nr12 nr21 ) ) . Defined .

Definition iscoantisymmresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : iscoantisymm L ) : iscoantisymm ( resrel L P ) .
...
Definition  neqchoiceresrel { X : Type } ( L : hrel X ) ( P : hsubtypes X ) ( isl : neqchoice L ) : neqchoice ( resrel L P ) .
...
Definition iseqclass { X : Type } ( R : hrel X ) ( A : hsubtypes X ) := dirprod ( ishinh ( carrier A ) ) ( dirprod ( ∀ x1 x2 : X , R x1 x2 -> A x1 -> A x2 ) ( ∀ x1 x2 : X, A x1 ->  A x2 -> R x1 x2 ) ).
Definition iseqclassconstr { X : Type } ( R : hrel X ) { A : hsubtypes X } ( ax0 : ishinh ( carrier A ) ) ( ax1 : ∀ x1 x2 : X , R x1 x2 -> A x1 -> A x2 ) ( ax2 : ∀ x1 x2 : X, A x1 ->  A x2 -> R x1 x2 ) : iseqclass R A := dirprodpair ax0 ( dirprodpair ax1 ax2 ) .

Definition eqax0 { X : Type } { R : hrel X } { A : hsubtypes X }  : iseqclass R A -> ishinh ( carrier A ) := fun is : iseqclass R A =>  pr1 is .
Definition eqax1 { X : Type } { R : hrel X } { A : hsubtypes X } : iseqclass R A ->  ∀ x1 x2 : X,  R x1 x2 -> A x1 -> A x2 := fun is: iseqclass R A => pr1 ( pr2 is) .
Definition eqax2 { X : Type } { R : hrel X } { A : hsubtypes X } : iseqclass R A ->  ∀ x1 x2 : X,  A x1 -> A x2 -> R x1 x2 := fun is: iseqclass R A => pr2 ( pr2 is) .

Lemma isapropiseqclass {X : Type} (R : hrel X) (A : hsubtypes X) :
...
Lemma iseqclassdirprod { X Y : Type } { R : hrel X } { Q : hrel Y } { A : hsubtypes X } { B : hsubtypes Y } ( isa : iseqclass R A ) ( isb : iseqclass Q B ) : iseqclass ( hreldirprod R Q ) ( subtypesdirprod A B ) .
...
Theorem surjectionisepitosets { X Y Z : Type } ( f : X -> Y ) ( g1 g2 : Y -> Z ) ( is1 : issurjective f ) ( is2 : isaset Z ) ( isf : ∀ x : X , g1 (f x) = g2 (f x)  ) : ∀ y : Y , g1 y = g2 y .
...
Definition setquot { X : Type } ( R : hrel X ) := total2 ( fun A : _ => iseqclass R A ) .
Definition setquotpair { X : Type } ( R : hrel X ) ( A : hsubtypes X ) ( is : iseqclass R A ) := tpair _ A is .
Definition pr1setquot { X : Type } ( R : hrel X ) : setquot R -> ( hsubtypes X ) := @pr1 _ ( fun A : _ => iseqclass R A ) .
Coercion pr1setquot : setquot >-> hsubtypes .

Lemma isinclpr1setquot {X : Type} (R : hrel X) : isincl (pr1setquot R).
Proof.
intros X R. apply isinclpr1. intro x0. apply isapropiseqclass.
Defined.

Definition setquottouu0 {X : Type} (R : hrel X) (a : setquot R) := carrier (pr1 a).
Coercion setquottouu0 : setquot >-> Sortclass.

Theorem isasetsetquot {X : Type} (R : hrel X) : isaset (setquot R).
Proof.
intros X R.
...
Definition setquotinset { X : Type } ( R : hrel X ) : hSet :=
  hSetpair _ ( isasetsetquot R ) .

Theorem setquotpr { X : Type } ( R : eqrel X ) : X -> setquot R.
Proof.
intros X R X0.
set (rax := eqrelrefl R).
set (sax := eqrelsymm R).
...
Lemma setquotl0 { X : Type } ( R : eqrel X ) ( c : setquot R ) ( x : c ) : setquotpr R ( pr1 x ) = c .
Proof.
intros X R c x.
...
Theorem issurjsetquotpr { X : Type } ( R : eqrel X)  : issurjective (setquotpr R ).
...
Lemma iscompsetquotpr { X : Type } ( R : eqrel X ) ( x x' : X ) ( a : R x x' ) : setquotpr R x = setquotpr R x' .
...
Definition iscomprelfun { X Y : Type } ( R : hrel X ) ( f : X -> Y ) := ∀ x x' : X , R x x' -> f x = f x' .

Lemma iscomprelfunlogeqf { X Y : Type } { R L : hrel X } ( lg : hrellogeq L R ) ( f : X -> Y ) ( is : iscomprelfun L f ) : iscomprelfun R f .
Proof . intros . intros x x' r . apply ( is _ _ ( pr2 ( lg  _ _ ) r ) ) . Defined .

Lemma isapropimeqclass { X : Type } ( R : hrel X ) ( Y : hSet ) ( f : X -> Y ) ( is : iscomprelfun R f ) ( c : setquot R ) : isaprop ( image ( fun x : c => f ( pr1 x ) ) ) .
...
Theorem setquotuniv  { X : Type } ( R : hrel X ) ( Y : hSet ) ( f : X -> Y ) ( is : iscomprelfun R f ) ( c : setquot R ) : Y .
...
Theorem setquotunivcomm  { X : Type } ( R : eqrel X ) ( Y : hSet ) ( f : X -> Y ) ( is : iscomprelfun R f ) : ∀ x : X , setquotuniv R Y f is ( setquotpr R x ) =f x .
Proof. intros. unfold setquotuniv . unfold setquotpr .  simpl .  apply idpath .  Defined.


Theorem weqpathsinsetquot { X : Type } ( R : eqrel X ) ( x x' : X ) : R x x' ≃ setquotpr R x = setquotpr R x' .
...
Definition iscomprelrelfun { X Y : Type } ( RX : hrel X ) ( RY : hrel Y ) ( f : X -> Y ) := ∀ x x' : X , RX x x' -> RY ( f x ) ( f x' ) .

Lemma iscomprelfunlogeqf1 { X Y : Type }  { LX RX : hrel X } ( RY : hrel Y ) ( lg : hrellogeq LX RX ) ( f : X -> Y ) ( is : iscomprelrelfun LX RY f ) : iscomprelrelfun RX RY f .
Proof . intros . intros x x' r . apply ( is _ _ ( pr2 ( lg  _ _ ) r ) ) . Defined .

Lemma iscomprelfunlogeqf2 { X Y : Type }  ( RX : hrel X ) { LY RY : hrel Y } ( lg : hrellogeq LY RY ) ( f : X -> Y ) ( is : iscomprelrelfun RX LY f ) : iscomprelrelfun RX RY f .
Proof . intros . intros x x' r . apply ( ( pr1 ( lg _ _ ) ) ( is _ _ r ) ) . Defined .

Definition  setquotfun  { X Y : Type } ( RX : hrel X ) ( RY : eqrel Y ) ( f : X -> Y ) ( is : iscomprelrelfun RX RY f ) ( cx : setquot RX ) : setquot RY .
...
Definition setquotfuncomm  { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( f : X -> Y ) ( is : iscomprelrelfun RX RY f ) : ∀ x : X , setquotfun RX RY f is ( setquotpr RX x ) = setquotpr RY ( f x ) .
...
Theorem setquotunivprop {X : Type} (R : eqrel X) (P : setquot (pr1 R) -> hProp)
...
Theorem setquotuniv2prop { X : Type } ( R : eqrel X ) ( P : setquot R -> setquot R -> hProp ) ( is : ∀ x x' : X ,  P ( setquotpr R x ) ( setquotpr R x' ) ) : ∀ c c' : setquot R ,  P c c' .
...
Theorem setquotuniv3prop { X : Type } ( R : eqrel X ) ( P : setquot R -> setquot R -> setquot R -> hProp ) ( is : ∀ x x' x'' : X ,  P  ( setquotpr R x ) ( setquotpr R x' ) ( setquotpr R x'' ) ) : ∀ c c' c'' : setquot R , P c c' c''  .
...
Theorem setquotuniv4prop { X : Type } ( R : eqrel X ) ( P : setquot R -> setquot R ->  setquot R -> setquot R -> hProp ) ( is : ∀ x x' x'' x''' : X ,  P  ( setquotpr R x ) ( setquotpr R x' ) ( setquotpr R x'' ) ( setquotpr R x''' ) ) : ∀ c c' c'' c''' : setquot R , P c c' c'' c''' .
...
Lemma issurjsetquotfun { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( f : X -> Y ) ( is : issurjective f ) ( is1 : iscomprelrelfun RX RY f ) : issurjective ( setquotfun RX RY f is1 ) .
Proof . intros . apply ( issurjtwooutof3b ( setquotpr RX ) ) . apply ( issurjcomp f ( setquotpr RY ) is ( issurjsetquotpr RY ) ) .   Defined .


Lemma isinclsetquotfun { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( f : X -> Y ) ( is1 : iscomprelrelfun RX RY f  )  ( is2 : ∀ x x' : X , RY ( f x ) ( f x' ) -> RX x x' ) : isincl ( setquotfun RX RY f is1 ) .
...
Definition setquotincl { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( f : X -> Y ) ( is1 : iscomprelrelfun RX RY f  )  ( is2 : ∀ x x' : X , RY ( f x ) ( f x' ) -> RX x x' ) := inclpair ( setquotfun RX RY f is1 ) ( isinclsetquotfun RX RY f is1 is2 ) .

Definition  weqsetquotweq { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( f : weq X Y ) ( is1 : iscomprelrelfun RX RY f  )  ( is2 : ∀ x x' : X , RY ( f x ) ( f x' ) -> RX x x' ) : weq ( setquot RX ) ( setquot RY )  .
...
Definition weqsetquotsurj  { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( f : X -> Y ) ( is : issurjective f ) ( is1 : iscomprelrelfun RX RY f  )  ( is2 : ∀ x x' : X , RY ( f x ) ( f x' ) -> RX x x' ) : weq ( setquot RX ) ( setquot RY )  .
...
Definition setquottodirprod { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( cc : setquot ( eqreldirprod RX RY ) ) : dirprod ( setquot RX ) ( setquot RY ) .
...
Definition dirprodtosetquot { X Y : Type } ( RX : hrel X ) ( RY : hrel Y ) (cd : dirprod ( setquot RX ) ( setquot RY ) ) : setquot ( hreldirprod RX RY ) := setquotpair _ _ ( iseqclassdirprod ( pr2 ( pr1 cd ) ) ( pr2 ( pr2 cd ) ) ) .


Theorem weqsetquottodirprod { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) : weq ( setquot ( eqreldirprod RX RY ) ) ( dirprod ( setquot RX ) ( setquot RY ) ) .
...
Definition iscomprelfun2 { X Y : Type } ( R : hrel X ) ( f : X -> X -> Y ) := ∀ x x' x0 x0' : X , R x x' ->  R x0 x0' -> f x x0 = f x' x0' .

Lemma iscomprelfun2if { X Y : Type } ( R : hrel X ) ( f : X -> X -> Y ) ( is1 : ∀ x x' x0 : X , R x x' -> f x x0 = f x' x0 ) ( is2 : ∀ x x0 x0' : X , R x0 x0' -> f x x0 = f x x0' ) : iscomprelfun2 R f .
Proof . intros . intros x x' x0 x0' .  intros r r' .  set ( e := is1 x x' x0 r ) . set ( e' := is2 x' x0 x0' r' ) . apply ( pathscomp0 e e' ) . Defined .

Lemma iscomprelfun2logeqf { X Y : Type } { L R : hrel X } ( lg : hrellogeq L R ) ( f : X -> X -> Y ) ( is : iscomprelfun2 L f ) : iscomprelfun2 R f .
Proof . intros . intros x x' x0 x0' r r0 . apply ( is _ _ _ _ ( ( pr2 ( lg _ _ ) ) r )  ( ( pr2 ( lg _ _ ) ) r0 ) ) . Defined .

Definition setquotuniv2  { X : Type } ( R : hrel X ) ( Y : hSet ) ( f : X -> X -> Y ) ( is : iscomprelfun2 R f ) ( c c0 : setquot R ) : Y .
...
Theorem setquotuniv2comm  { X : Type } ( R : eqrel X ) ( Y : hSet ) ( f : X -> X -> Y ) ( is : iscomprelfun2 R f ) : ∀ x x' : X , setquotuniv2 R Y f is ( setquotpr R x ) ( setquotpr R x' ) = f x x' .
...
Definition iscomprelrelfun2 { X Y : Type } ( RX : hrel X ) ( RY : hrel Y ) ( f : X -> X -> Y ) := ∀ x x' x0 x0' : X , RX x x' -> RX x0 x0' ->  RY ( f x x0 ) ( f x' x0' ) .

Lemma iscomprelrelfun2if { X Y : Type } ( RX : hrel X ) ( RY : eqrel Y ) ( f : X -> X -> Y ) ( is1 : ∀ x x' x0 : X , RX x x' -> RY ( f x x0 ) ( f x' x0 ) ) ( is2 : ∀ x x0 x0' : X , RX x0 x0' -> RY ( f x x0 ) ( f x x0' ) ) : iscomprelrelfun2 RX RY f .
Proof . intros . intros x x' x0 x0' .  intros r r' .  set ( e := is1 x x' x0 r ) . set ( e' := is2 x' x0 x0' r' ) . apply ( eqreltrans RY _ _ _ e e' ) . Defined .

Lemma iscomprelrelfun2logeqf1 { X Y : Type } { LX RX : hrel X } ( RY : hrel Y ) ( lg : hrellogeq LX RX ) ( f : X -> X -> Y ) ( is : iscomprelrelfun2 LX RY f ) : iscomprelrelfun2 RX RY f .
Proof . intros . intros x x' x0 x0' r r0 . apply ( is _ _ _ _ ( ( pr2 ( lg _ _ ) ) r )  ( ( pr2 ( lg _ _ ) ) r0 ) ) . Defined .

Lemma iscomprelrelfun2logeqf2 { X Y : Type } ( RX : hrel X ) { LY RY : hrel Y } ( lg : hrellogeq LY RY ) ( f : X -> X -> Y ) ( is : iscomprelrelfun2 RX LY f ) : iscomprelrelfun2 RX RY f .
Proof . intros . intros x x' x0 x0' r r0 . apply ( ( pr1 ( lg _ _ ) ) ( is _ _ _ _ r r0 ) ) .  Defined .

Definition  setquotfun2  { X Y : Type } ( RX : hrel X ) ( RY : eqrel Y ) ( f : X -> X -> Y ) ( is : iscomprelrelfun2 RX RY f ) ( cx cx0 : setquot RX ) : setquot RY .
...
Theorem setquotfun2comm  { X Y : Type } ( RX : eqrel X ) ( RY : eqrel Y ) ( f : X -> X -> Y ) ( is : iscomprelrelfun2 RX RY f ) : ∀ x x' : X , setquotfun2 RX RY f is ( setquotpr RX x ) ( setquotpr RX x' ) =  setquotpr RY ( f x x' ) .
Proof. intros.   apply idpath .  Defined.
...
Theorem isdeceqsetquot_non_constr { X : Type } ( R : eqrel X ) ( is : ∀ x x' : X , isdecprop ( R x x' ) ) : isdeceq ( setquot R ) .
...
Definition setquotbooleqint { X : Type } ( R : eqrel X ) ( is : ∀ x x' : X , isdecprop ( R x x' ) ) ( x x' : X ) : bool .
Proof . intros . destruct ( pr1 ( is x x' ) ) . apply true . apply false . Defined .

Lemma  setquotbooleqintcomp { X : Type } ( R : eqrel X ) ( is : ∀ x x' : X , isdecprop ( R x x' ) ) : iscomprelfun2 R ( setquotbooleqint R is ) .
...
Definition setquotbooleq { X : Type } ( R : eqrel X ) ( is : ∀ x x' : X , isdecprop ( R x x' ) ) : setquot R -> setquot R -> bool := setquotuniv2 R ( hSetpair _ ( isasetbool ) ) ( setquotbooleqint R is ) ( setquotbooleqintcomp R is ) .

Lemma setquotbooleqtopaths  { X : Type } ( R : eqrel X ) ( is : ∀ x x' : X , isdecprop ( R x x' ) ) ( x x' : setquot R ) : setquotbooleq R is x x' = true  -> x = x' .
...
Lemma setquotpathstobooleq  { X : Type } ( R : eqrel X ) ( is : ∀ x x' : X , isdecprop ( R x x' ) ) ( x x' : setquot R ) : x = x' -> setquotbooleq R is x x' = true .
...
Definition  isdeceqsetquot { X : Type } ( R : eqrel X ) ( is : ∀ x x' : X , isdecprop ( R x x' ) ) : isdeceq ( setquot R ) .
...
Definition iscomprelrel { X : Type } ( R : hrel X ) ( L : hrel X ) := iscomprelfun2 R L .

Lemma iscomprelrelif { X : Type } { R : hrel X } ( L : hrel X ) ( isr : issymm R ) ( i1 : ∀ x x' y , R x x' -> L x y -> L x' y ) ( i2 : ∀ x y y' , R y y' -> L x y -> L x y' ) : iscomprelrel R L .
...
Lemma iscomprelrellogeqf1 { X : Type } { R R' : hrel X } ( L : hrel X ) ( lg : hrellogeq R R' ) ( is : iscomprelrel R L ) : iscomprelrel R' L .
Proof . intros . apply ( iscomprelfun2logeqf lg L is ) .  Defined .

Lemma iscomprelrellogeqf2 { X : Type } ( R : hrel X ) { L L' : hrel X } ( lg : hrellogeq L L' ) ( is : iscomprelrel R L ) : iscomprelrel R L' .
...
Definition quotrel  { X : Type } { R L : hrel X } ( is : iscomprelrel R L ) : hrel ( setquot R ) := setquotuniv2 R hPropset L is .

Lemma istransquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : istrans L ) : istrans ( quotrel is ) .
...
Lemma issymmquotrel  { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : issymm L ) : issymm ( quotrel is ) .
...
Lemma isreflquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : isrefl L ) : isrefl ( quotrel is ) .
Proof . intros . unfold isrefl .  apply ( setquotunivprop R ) .   intro x .  apply ( isl x ) . Defined .

Lemma ispoquotrel  { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : ispreorder L ) : ispreorder ( quotrel is ) .
Proof . intros . split with ( istransquotrel is ( pr1 isl ) ) .  apply ( isreflquotrel is ( pr2 isl ) ) .  Defined .

Lemma iseqrelquotrel  { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : iseqrel L ) : iseqrel ( quotrel is ) .
Proof . intros . split with ( ispoquotrel is ( pr1 isl ) ) .  apply ( issymmquotrel is ( pr2 isl ) ) .  Defined .

Lemma isirreflquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : isirrefl L ) : isirrefl ( quotrel is ) .
...
Lemma isasymmquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : isasymm L ) : isasymm ( quotrel is ) .
...
Lemma iscoasymmquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : iscoasymm L ) : iscoasymm ( quotrel is ) .
...
Lemma istotalquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : istotal L ) : istotal ( quotrel is ) .
Proof . intros .  unfold istotal . apply ( setquotuniv2prop R ( fun x1 x2 => hdisj _ _ ) ) .  intros x x' . intros r r' . apply ( isl x x' r r' ) . Defined .

Lemma iscotransquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : iscotrans L ) : iscotrans ( quotrel is ) .
...
Lemma isantisymmquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : isantisymm L ) : isantisymm ( quotrel is ) .
...
Lemma isantisymmnegquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : isantisymmneg L ) : isantisymmneg ( quotrel is ) .
...
Lemma quotrelimpl { X : Type } { R : eqrel X } { L L' : hrel X } ( is : iscomprelrel R L ) ( is' : iscomprelrel R L' ) ( impl : ∀ x x' , L x x' -> L' x x' ) ( x x' : setquot R ) ( ql : quotrel is x x' ) : quotrel is' x x'  .
...
Lemma quotrellogeq { X : Type } { R : eqrel X } { L L' : hrel X } ( is : iscomprelrel R L ) ( is' : iscomprelrel R L' ) ( lg : ∀ x x' , L x x' <-> L' x x' ) ( x x' : setquot R ) : ( quotrel is x x' ) <-> ( quotrel is' x x' ) .
...
Definition quotdecrelint { X : Type } { R : hrel X } ( L : decrel X ) ( is : iscomprelrel R ( pr1 L ) )  : brel ( setquot R ) .
...
Definition quotdecrelintlogeq { X : Type } { R : eqrel X } ( L : decrel X ) ( is : iscomprelrel R ( pr1 L ) ) ( x x' : setquot R ) : breltodecrel ( quotdecrelint L is ) x x' <-> quotrel is x x' .
...
Lemma isdecquotrel { X : Type } { R : eqrel X } { L : hrel X } ( is : iscomprelrel R L ) ( isl : isdecrel L ) : isdecrel ( quotrel is ) .
...
Definition decquotrel  { X : Type } { R : eqrel X } ( L : decrel X ) ( is : iscomprelrel R L ) : decrel ( setquot R ) := decrelpair ( isdecquotrel is ( pr2 L ) ) .
...
Definition reseqrel { X : Type } ( R : eqrel X ) ( P : hsubtypes X ) : eqrel P := eqrelpair _ ( iseqrelresrel R P ( pr2 R ) ) .

Lemma iseqclassresrel { X : Type } ( R : hrel X ) ( P Q : hsubtypes X ) ( is : iseqclass R Q ) ( is' : ∀ x , Q x -> P x ) : iseqclass ( resrel R P ) ( fun x : P => Q ( pr1 x ) ) .
Proof . intros . split .
...
Definition fromsubquot { X : Type } ( R : eqrel X ) ( P : hsubtypes ( setquot R ) ) ( p : P )  : setquot ( resrel R ( funcomp ( setquotpr R ) P ) ) .
...
Definition tosubquot { X : Type } ( R : eqrel X ) ( P : hsubtypes ( setquot R ) ) : setquot ( resrel R ( funcomp ( setquotpr R ) P ) ) -> P .
...
Definition weqsubquot { X : Type } ( R : eqrel X ) ( P : hsubtypes ( setquot R ) ) : weq P ( setquot ( resrel R ( funcomp ( setquotpr R ) P ) ) ) .
...
[ Lemma test ( X : Type ) ( R : eqrel X ) ( P : hsubtypes ( setquot R ) ) ( x : X ) ( is : P ( setquotpr R x ) ) : paths ( setquotpr ( reseqrel R (funcomp (setquotpr R) P) ) ( tpair _ x is ) ) ( fromsubquot R P ( tpair _ ( setquotpr R x ) is ) ) .
Proof . intros . apply idpath . Defined . ]
...
Lemma proofirrelevance { X : Type } ( is : isaprop X ) : ∀ x1 x2 , paths x1 x2 .
...
Definition pathshrel ( X : Type ) := fun x x' : X  =>  ishinh ( x = x')  .
Definition istranspathshrel ( X : Type ) : istrans ( pathshrel X ) := fun x x' x'' : _ => fun a : _ => fun b : _ =>  hinhfun2 (fun e1 : x = x' => fun e2 : x' = x'' => e1 @ e2 ) a b .
Definition isreflpathshrel ( X : Type ) : isrefl ( pathshrel X ) := fun x : _ =>  hinhpr ( idpath x ) .
Definition issymmpathshrel ( X : Type ) : issymm ( pathshrel X ) := fun x x': _ => fun a : _ => hinhfun ( fun e : x = x' => ! e ) a .

Definition pathseqrel ( X : Type ) := eqrelconstr ( pathshrel X ) ( istranspathshrel X ) ( isreflpathshrel X ) ( issymmpathshrel X ) .

Definition pi0 ( X : Type ) := setquot ( pathshrel X ) .
Definition pi0pr ( X : Type ) := setquotpr ( pathseqrel X ) .
...
Definition compfun { X : Type }  ( R : hrel X ) ( S : Type ) : Type := total2  (fun F: X -> S => iscomprelfun R F ) .
Definition compfunpair { X : Type }  ( R : hrel X ) { S : Type } ( f : X -> S ) ( is : iscomprelfun R f ) : compfun R S := tpair _ f is .
Definition pr1compfun ( X : Type )  ( R : hrel X ) ( S : Type ) : @compfun X R S -> ( X -> S ) := @pr1 _ _ .
Coercion pr1compfun : compfun >-> Funclass .

Definition compevmapset { X : Type } ( R : hrel X ) : X -> ∀ S : hSet, ( compfun R S ) -> S := fun x : X => fun S : _ => fun f : compfun R S => pr1 f x.

Definition compfuncomp { X : Type }  ( R : hrel X ) { S S' : Type } ( f : compfun R S ) ( g : S -> S' ) : compfun R S' .
Proof . intros . split with ( funcomp f g ) . intros x x' r .  apply ( maponpaths g ( pr2 f x x' r ) ) . Defined .


(** Tests

Definition F ( X Y : Type ) ( R : hrel X ) := ( compfun R Y ) -> Y .

Definition Fi ( X Y : Type ) ( R : hrel X ) : X -> F X Y R := fun x => fun f => f x .

Lemma iscompFi { X Y : Type } ( R : hrel X ) : iscomprelfun R ( Fi X Y R ) .
Proof . intros . intros x x' r . unfold Fi . apply funextfun .  intro f . apply ( pr2 f x x' r ) .  Defined .

Definition Fv { X Y : Type } ( R : hrel X ) ( f : compfun R Y ) ( phi : F X Y R ) : Y := phi f .

Definition qeq { X Y : Type } ( R : hrel X ) := total2 ( fun phi : F X Y R => ∀ psi : F X Y R -> Y  , paths ( psi phi ) ( Fv R ( compfuncomp R ( compfunpair R _ ( iscompFi R ) ) psi ) phi ) ) .

Lemma isinclpr1qeq { X : Type } ( R : hrel X ) ( Y : hSet ) : isincl ( @pr1 _ ( fun phi : F X Y R => ∀ psi : F X Y R -> Y  , paths ( psi phi ) ( Fv R ( compfuncomp R ( compfunpair R _ ( iscompFi R ) ) psi ) phi ) ) ) .
Proof . intros . apply isinclpr1 .  intro phi .  apply impred . intro psi .  apply ( pr2 Y ) . Defined.

Definition toqeq { X Y : Type } ( R : hrel X ) ( x : X ) : @qeq X Y R .
Proof . intros . split with ( Fi X Y R x ) . intro psi. apply idpath . Defined .

Lemma iscomptoqeq  { X : Type } ( Y : hSet ) ( R : hrel X ) : iscomprelfun R ( @toqeq X Y R ) .
Proof . intros . intros x x' r . unfold toqeq . apply ( invmaponpathsincl _ ( isinclpr1qeq R Y ) ) . apply (  @iscompFi X Y R x x' r ) . Defined .

Definition qequniv { X : Type } ( Y : hSet ) ( R : hrel X ) ( f : compfun R Y ) ( phi : @qeq X Y R ) : Y .
Proof . intros . apply ( Fv R f ( pr1 phi ) ) . Defined.

Lemma qequnivandpr { X : Type } ( Y : hSet ) ( R : hrel X ) ( f : compfun R Y ) ( x : X ) : paths ( qequniv Y R f ( toqeq R x ) ) ( f x ) .
Proof . intros . apply idpath . Defined .

Lemma etaqeq { X : Type } ( Y : hSet ) ( R : hrel X ) ( psi : qeq R -> Y ) ( phi : qeq R ) : paths ( psi phi ) ( qequniv Y R ( compfuncomp R ( compfunpair R _ ( iscomptoqeq Y R ) ) psi ) phi ) .
Proof .  intros . apply ( pr2 phi psi ) .




Definition Fd1 { X Y : Type } : F X Y R -> ( F ( F X Y ) Y ) := Fi ( F X Y ) Y .

Definition Fd2 { X Y : Type } ( R : hrel X ) ( phi : F X Y R ) ( psi : F X Y R -> Y ) : Y := ( Fv R ( funcomp ( Fi X Y R ) psi ) phi ) .

Definition Ffunct { X1 X2 : Type } ( f : X1 -> X2 ) ( Y : Type ) : F X1 Y -> F X2 Y := fun phi => fun g => phi ( funcomp f g ) .



Lemma testd1 { X Y : Type } ( psi : F X Y -> Y ) ( phi : F X Y ) : paths ( psi phi ) ( Fd1 phi psi )  .
Proof . intros . apply idpath . Defined .

Lemma testd2 { X Y : Type } ( psi : F X Y -> Y ) ( phi : F X Y ) : paths ( Fv ( funcomp ( Fi X Y ) psi ) phi ) ( Fd2 phi psi )  .
Proof . intros . apply idpath . Defined .

Definition F ( X Y : Type ) := ( X -> Y ) -> Y .

Definition Ffunct { X1 X2 : Type } ( f : X1 -> X2 ) ( Y : Type ) : F X1 Y -> F X2 Y := fun phi => fun g => phi ( funcomp f g ) .

Definition Fi ( X Y : Type ) : X -> F X Y := fun x => fun f => f x .

Definition Fd1 { X Y : Type } : F X Y -> ( F ( F X Y ) Y ) := Fi ( F X Y ) Y .

Definition Fd2 { X Y : Type } : F X Y -> ( F ( F X Y ) Y ) := Ffunct ( Fi X Y ) Y .

Definition Fv { X Y : Type } ( f : X -> Y ) ( phi : F X Y ) : Y := phi f .
...
Lemma Xineq ( X Y : Type ) ( x : X ) : paths ( Fd1 ( Fi X Y x ) ) ( Fd2 ( Fi X Y x ) ) .
Proof . intros . apply idpath . Defined .

Lemma test ( X Y : Type ) ( phi : F X Y ) ( f : X -> Y ) : paths ( Fd1 phi ( Fi ( X -> Y ) Y f ) ) ( Fd2 phi ( Fi ( X -> Y ) Y f ) ) .
...
Inductive try0 ( T : Type ) ( t : T ) : ∀ ( t1 t2 : T ) ( e1 : paths t t1 ) ( e2 : paths t t2 ) , Type := idconstr : ∀ ( t' : T ) ( e' : paths t t' ) , try0 T t t' t' e' e' .
...
Lemma test ( X : Type ) ( t : X ) : paths ( pr2 ( iscontrcoconustot X t ) (  pr1 ( iscontrcoconustot X t ) ) ) ( idpath _ ) .
Proof . intros . apply idpath .


Lemma test { X : Type } ( is : iscontr X ) : paths ( pr2 ( iscontrcor is ) ( pr1 ( iscontrcor is ) ) ) ( idpath _ ) .
Proof . intros . apply idpath .




Lemma test { X : Type } ( R : eqrel X ) ( Y : hSet ) ( f : setquot R -> Y ) : paths f ( setquotuniv R Y ( funcomp ( setquotpr R ) f ) ( fun x x' : X => fun r : R x x' => maponpaths f ( iscompsetquotpr R x x' r ) ) ) .
...
Definition setquot2 { X : Type } ( R : hrel X ) : Type := image  ( compevmapset R ) .

Theorem isasetsetquot2 { X : Type } ( R : hrel X ) : isaset ( setquot2 R ) .
Proof. intros.
...
Definition setquot2inset { X : Type } ( R : hrel X ) : hSet := hSetpair _ ( isasetsetquot2 R ) .

(** We will be asuming below that setquot2 is in Type.  In the future it should be proved using [ issurjsetquot2pr ] below and a resizing axiom. The appropriate resizing axiom for this should say that if X -> Y is a surjection, Y is an hset and X : Type then Y : Type . *)

Definition setquot2pr { X : Type }  ( R : hrel X ) : X -> setquot2 R := fun x : X => imagepair ( compevmapset R ) _ ( hinhpr ( hfiberpair ( compevmapset R ) x ( idpath _ ) ) ) .

Lemma issurjsetquot2pr { X : Type } ( R : hrel X ) : issurjective ( setquot2pr R ) .
Proof. intros. apply issurjprtoimage. Defined.

Lemma iscompsetquot2pr { X : Type } ( R : hrel X ) : iscomprelfun R ( setquot2pr R ) .
...
Definition setquot2univ { X : Type } ( R : hrel X ) ( Y : hSet ) ( F : X -> Y ) (is : iscomprelfun R F ) ( c: setquot2 R ) : Y := pr1 c Y ( compfunpair _ F is ) .

Theorem setquot2univcomm  { X : Type } ( R : hrel X ) ( Y : hSet ) ( F : X -> Y ) (iscomp : iscomprelfun R F ) ( x : X) : setquot2univ _ _ F iscomp ( setquot2pr R x ) = F x .
Proof. intros. apply idpath. Defined.

(** *** Weak equivalence from [ R x x' ] to [ paths ( setquot2pr R x ) ( setquot2pr R x' ) ] *)

Lemma weqpathssetquot2l1 { X : Type } ( R : eqrel X ) ( x : X ) : iscomprelfun R ( fun x' => R x x' ) .
...
Theorem weqpathsinsetquot2 { X : Type } ( R : eqrel X ) ( x x' : X ) : weq ( R x x' ) ( setquot2pr R x = setquot2pr R x' ) .
...
Definition setquottosetquot2 (X: Type) (R: hrel X) (is: iseqrel R) : setquot R -> setquot2 R.
