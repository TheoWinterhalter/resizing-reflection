...
Axiom funextempty : ∀ (X:Type) (f g : X->empty), f = g.
...
Corollary isapropdneg (X:Type): isaprop (dneg X).
Proof. intro. apply (isapropneg (neg X)). Defined.


Definition isaninvprop (X:Type) := isweq  (todneg X).

Definition invimpl (X:Type) (is: isaninvprop X) : (dneg X) -> X:= invmap  ( weqpair (todneg X) is ) .


Lemma isapropaninvprop (X:Type): isaninvprop X -> isaprop X.
Proof. intros X X0.
apply (isofhlevelweqb (S O) ( weqpair (todneg X) X0 ) (isapropdneg X)). Defined.


Theorem isaninvpropneg (X:Type): isaninvprop (neg X).
Proof. intros.
...
Theorem isapropdec (X:Type): isaprop X -> isaprop (X ⨿ ¬X).
(* uses [funextempty] *)
Proof.
...
Definition compl (X:Type) (x:X):= Σ x', x != x'.
Definition complpair ( X : Type ) ( x : X ) := tpair (fun x':X => neg (paths x x' ) ) .
Definition pr1compl ( X : Type ) ( x : X ) := @pr1 _ (fun x':X => neg (paths x x' ) ) .


Lemma isinclpr1compl ( X : Type ) ( x : X ) : isincl ( pr1compl X x ) .
...
Coercion negProp_to_type : negProp >-> Type.
...
Definition negPred {X:Type} (x  :X) (P:∀ y:X, Type)      := ∀ y  , negProp (P y).

Definition negReln {X:Type}         (P:∀ (x y:X), Type)  := ∀ x y, negProp (P x y).

Definition neqProp {X:Type} (x y:X) :=            negProp (x=y).

Definition neqPred {X:Type} (x  :X) := ∀ y,       negProp (x=y).

Definition neqReln (X:Type)         := ∀ (x y:X), negProp (x=y).

Definition compl_ne (X:Type) (x:X) (neq_x : neqPred x) := Σ y, neq_x y.

Definition compl_ne_pair ( X : Type ) ( x : X ) (neq_x : neqPred x) (y:X) (ne:neq_x y) : compl_ne X x neq_x
  := (y,,ne).

Definition pr1compl_ne ( X : Type ) ( x : X ) (neq_x : neqPred x) (c:compl_ne X x neq_x) : X
  := pr1 c.
...
Definition make_neqProp {X:Type} (x y:X) : neqProp x y.
Proof. intros. apply make_negProp. Defined.

Lemma isinclpr1compl_ne ( X : Type ) ( x : X ) (neq_x : neqPred x) : isincl ( pr1compl_ne X x neq_x ) .
Proof. intros. apply isinclpr1. intro y. apply negProp_to_isaprop. Defined.

Lemma compl_ne_weq_compl ( X : Type ) ( x : X ) (neq_x : neqPred x) : compl X x ≃ compl_ne X x neq_x.
Proof.
  (* uses [funextempty] *)
...
Lemma compl_weq_compl_ne ( X : Type ) ( x : X ) (neq_x : neqPred x) : compl_ne X x neq_x ≃ compl X x.
Proof.
  (* uses [funextempty] *)
  intros. apply weqfibtototal; intro y. apply weqiff.
...
Definition recompl (X:Type) (x:X): compl X x ⨿ unit -> X
  := fun u:_ =>
       match u with
...
Definition recompl_ne (X:Type) (x:X) (neq_x:neqPred x) : compl_ne X x neq_x ⨿ unit -> X.
Proof.
  intros ? ? ? w.
  induction w as [c|t].
...
Definition maponcomplincl { X Y : Type } (f:X -> Y) (is: isincl f) (x:X) : compl X x -> compl Y (f x)
  := fun x0':_ =>
       match x0' with
         tpair _ x' neqx => tpair _ (f x') (negf  (invmaponpathsincl  _ is x x' ) neqx)
       end.

Definition maponcomplincl_ne { X Y : Type } (f:X -> Y) (is: isincl f) (x:X)
...
Definition weqoncompl { X Y : Type } (w: X ≃ Y) x : compl X x ≃ compl Y (w x).
Proof.
  (* uses [funextempty] *)
...
Definition weqoncompl_ne { X Y : Type } (w: X ≃ Y) x (neq_x : neqPred x) (neq_wx : neqPred (w x))
...
Definition weqoncompl_compute { X Y : Type } (w: weq X Y) ( x : X ) : ∀ x', pr1 (weqoncompl w x x') = w (pr1 x').
Proof. intros. induction x' as [x' b]. reflexivity. Defined.

Definition weqoncompl_ne_compute { X Y : Type }
           (w: X ≃ Y) ( x : X ) (neq_x : neqPred x) (neq_wx : neqPred (w x)) x' :
  pr1 (weqoncompl_ne w x neq_x neq_wx x') = w (pr1 x').
Proof. reflexivity. Defined.

Definition homotweqoncomplcomp { X Y Z : Type } ( f : weq X Y ) ( g : weq Y Z ) ( x : X ) : homot ( weqcomp ( weqoncompl f x ) ( weqoncompl g ( f x ) ) ) ( weqoncompl  ( weqcomp f g ) x ) .
...
Definition isisolated (X:Type) (x:X) := ∀ x':X, (x = x') ⨿ (x != x').
Definition isisolated_ne (X:Type) (x:X) (neq_x:neqPred x) := ∀ y:X, (x=y) ⨿ neq_x y.
...
Definition isolated ( T : Type ) := Σ t:T, isisolated _ t.
Definition isolated_ne ( T : Type ) (neq:neqReln T) := Σ t:T, isisolated_ne _ t (neq t).

Definition isolatedpair ( T : Type ) (t:T) (i:isisolated _ t) : isolated T
  := (t,,i).
Definition isolatedpair_ne ( T : Type ) (t:T) (neq:neqReln T) (i:isisolated_ne _ t (neq t)) : isolated_ne T neq
  := (t,,i).

Definition pr1isolated ( T : Type ) (x:isolated T) : T := pr1 x.
Definition pr1isolated_ne ( T : Type ) (neq:neqReln T) (x:isolated_ne T neq) : T := pr1 x.

Theorem isaproppathsfromisolated ( X : Type ) ( x : X ) ( is : isisolated X x ) : ∀ x', isaprop(x = x') .
...
Theorem isaproppathsfromisolated_ne (X:Type) (x:X) (neq_x:neqPred x) (is:isisolated_ne X x neq_x) (y:X)
  : isaprop (x=y).
Proof.
...
Theorem isaproppathstoisolated  ( X : Type ) ( x : X ) ( is : isisolated X x ) : ∀ x' : X, isaprop ( x' = x ) .
Proof . intros . apply ( isofhlevelweqf 1 ( weqpathsinv0 x x' ) ( isaproppathsfromisolated X x is x' ) ) . Defined .

Lemma isisolatedweqf { X Y : Type } (f : X ≃ Y) (x:X) : isisolated X x -> isisolated Y (f x).
Proof.
...
Theorem isisolatedinclb { X Y : Type } ( f : X -> Y ) ( is : isincl f ) ( x : X ) ( is0 : isisolated _ ( f x ) ) : isisolated _ x .
...
Lemma disjointl1 (X:Type): isisolated (coprod X unit) (ii2  tt).
...
Definition invrecompl (X:Type)(x:X)(is: isisolated X x): X -> coprod (compl X x) unit:=
fun x':X => match (is x') with
ii1 e => ii2  tt|
ii2 phi => ii1  (complpair _ _ x' phi)
end.

Definition invrecompl_ne (X:Type)(x:X)(neq_x:neqPred x)(is: isisolated X x): X -> compl_ne X x neq_x ⨿ unit.
Proof.
...
Theorem isweqrecompl_ne (X:Type) (x:X) (is:isisolated X x) (neq_x:neqPred x): isweq (recompl_ne _ x neq_x).
Proof.
...
Definition weqrecompl_ne (X:Type) (x:X) (is:isisolated X x) (neq_x:neqPred x): compl_ne X x neq_x ⨿ unit ≃ X
  := weqpair _ (isweqrecompl_ne X x is neq_x).

Theorem isweqrecompl (X:Type)(x:X)(is:isisolated X x): isweq (recompl _ x).
...
Definition weqrecompl (X : Type) (x:X) (is:isisolated _ x): compl X x ⨿ unit ≃ X
  := weqpair _ ( isweqrecompl X x is ) .

(** *** Theorem saying that [ recompl ] commutes up to homotopy with [ maponcomplweq ] *)

Theorem homotrecomplnat {X Y : Type} (w : X ≃ Y) (x : X) (a : compl X x ⨿ unit) :
...
Definition recomplf { X Y : Type } ( x : X ) ( y : Y ) ( isx : isisolated X x ) ( f : compl X x -> compl Y y )  := funcomp ( funcomp ( invmap ( weqrecompl X x isx ) ) ( coprodf f ( idfun unit ) ) )  ( recompl Y y ) .

Definition weqrecomplf { X Y : Type } ( x : X ) ( y : Y ) ( isx : isisolated X x ) ( isy : isisolated Y y ) ( w : weq ( compl X x ) ( compl Y y ) ) := weqcomp ( weqcomp ( invweq ( weqrecompl X x isx ) ) ( weqcoprodf w ( idweq unit ) ) ) ( weqrecompl Y y isy ) .

Definition homotrecomplfhomot { X Y : Type } ( x : X ) ( y : Y ) ( isx : isisolated X x ) ( f f' : compl X x -> compl Y y ) ( h : homot f f' ) : homot ( recomplf x y isx f ) ( recomplf x y isx f') .
...
Lemma pathsrecomplfxtoy { X Y : Type } ( x : X ) ( y : Y ) ( isx : isisolated X x ) ( f : compl X x -> compl Y y ) : paths ( recomplf x y isx f x ) y .
...
Definition homotrecomplfcomp { X Y Z : Type } ( x : X ) ( y : Y ) ( z : Z ) ( isx : isisolated X x ) ( isy : isisolated Y y ) ( f :  compl X x -> compl Y y )  ( g :  compl Y y -> compl Z z ) : homot ( funcomp ( recomplf x y isx f ) ( recomplf y z isy g ) ) ( recomplf x z isx ( funcomp f g ) ) .
...
Definition homotrecomplfidfun { X : Type } ( x : X ) ( isx : isisolated X x ) : homot ( recomplf x x isx ( idfun ( compl X x ) ) ) ( idfun _ ) .
...
Lemma ishomotinclrecomplf { X Y : Type } ( x : X ) ( y : Y ) ( isx : isisolated X x ) ( f : compl X x -> compl Y y ) ( x'n : compl X x ) ( y'n : compl Y y ) ( e : paths ( recomplf x y isx f ( pr1 x'n ) ) ( pr1 y'n ) ) : paths ( f x'n ) y'n .
...
Definition funtranspos0 {T:Type} (t1 t2 : T) (is2 : isisolated T t2) (x : compl T t1) :
  compl T t2
...
Definition homottranspos0t2t1t1t2 { T : Type } ( t1 t2 : T )
...
Definition weqtranspos0 { T : Type } ( t1 t2 : T ) :
...
Definition funtranspos { T : Type } ( t1 t2 : isolated T )  : T -> T := recomplf ( pr1 t1 ) ( pr1 t2 ) ( pr2 t1 ) ( funtranspos0 ( pr1 t1 ) ( pr1 t2 ) ( pr2 t2 ) ) .

Definition homottranspost2t1t1t2  { T : Type } ( t1 t2 : T ) ( is1 : isisolated T t1 ) ( is2 : isisolated T t2 ) : homot ( funcomp ( funtranspos ( tpair _ t1 is1 ) ( tpair _ t2 is2 ) ) ( funtranspos ( tpair _ t2 is2 ) ( tpair _ t1 is1 ) ) ) ( idfun _ ) .
...
Theorem weqtranspos { T : Type } ( t1 t2 : T ) ( is1 : isisolated T t1 ) ( is2 : isisolated T t2 ) : weq T T .
...
Lemma pathsfuntransposoft1 { T : Type } ( t1 t2 : T ) ( is1 : isisolated T t1  ) ( is2 : isisolated T t2 ) : paths ( funtranspos ( tpair _ t1 is1 ) ( tpair _ t2 is2 ) t1 ) t2 .
Proof . intros . unfold funtranspos . rewrite ( pathsrecomplfxtoy t1 t2 is1 _ ) . apply idpath .  Defined .

Lemma pathsfuntransposoft2 { T : Type } ( t1 t2 : T ) ( is1 : isisolated T t1 ) ( is2 : isisolated T t2 ) : paths ( funtranspos ( tpair _ t1 is1 ) ( tpair _ t2 is2 ) t2 ) t1 .
...
Lemma pathsfuntransposofnet1t2 { T : Type } ( t1 t2 : T ) ( is1 : isisolated T t1 ) ( is2 : isisolated T t2 ) ( t : T ) ( net1t : neg ( paths t1 t ) ) ( net2t : neg ( paths t2 t ) ) : paths ( funtranspos ( tpair _ t1 is1 ) ( tpair _ t2 is2 ) t ) t .
...
Lemma homotfuntranspos2 { T : Type } ( t1 t2 : T ) ( is1 : isisolated T t1 ) ( is2 : isisolated T t2 ) : homot ( funcomp ( funtranspos ( tpair _ t1 is1 ) ( tpair _ t2 is2 ) ) ( funtranspos ( tpair _ t1 is1 ) ( tpair _ t2 is2 ) ) ) ( idfun _ ) .
...
Definition isdeceq (X:Type) : Type := ∀ (x x':X), (x=x') ⨿ (x!=x').

Lemma isdeceqweqf { X Y : Type } ( w : weq X Y ) ( is : isdeceq X ) : isdeceq Y .
...
Lemma isdeceqweqb { X Y : Type } ( w : weq X Y ) ( is : isdeceq Y ) : isdeceq X .
Proof . intros . apply ( isdeceqweqf ( invweq w ) is ) . Defined .

Theorem isdeceqinclb { X Y : Type } ( f : X -> Y ) ( is : isdeceq Y ) ( is' : isincl f ) : isdeceq X .
...
Lemma isdeceqifisaprop ( X : Type ) : isaprop X -> isdeceq X .
Proof. intros X is . intros x x' . apply ( ii1 ( proofirrelevance _ is x x' ) ) .  Defined .

Theorem isasetifdeceq (X:Type): isdeceq X -> isaset X.
Proof. intro X . intro is. intros x x' . apply ( isaproppathsfromisolated X x ( is x ) ) .   Defined .



Definition booleq { X : Type } ( is : isdeceq X ) ( x x' : X ) : bool .
Proof . intros . induction ( is x x' ) . apply true . apply false . Defined .


Lemma eqfromdnegeq (X:Type)(is: isdeceq X)(x x':X): dneg ( paths x x' ) -> paths x x'.
...
Definition subsetsplit { X : Type } ( f : X -> bool ) ( x : X ) : coprod ( hfiber f true ) ( hfiber f false ) .
Proof . intros . induction ( boolchoice ( f x ) ) as [ a | b ] .  apply ( ii1 ( hfiberpair f x a ) ) . apply ( ii2 ( hfiberpair f x b ) ) .  Defined .

Definition subsetsplitinv { X : Type } ( f : X -> bool ) ( ab : coprod (hfiber f true) (hfiber f false) )  : X :=  match ab with ii1 xt => pr1  xt | ii2 xf => pr1  xf end.


Theorem weqsubsetsplit { X : Type } ( f : X -> bool ) : weq X (coprod ( hfiber f true) ( hfiber f false) ) .
...
Definition eqbx ( X : Type ) ( x : X ) ( is : isisolated X x ) : X -> bool .
Proof. intros X x is x' . induction ( is x' ) . apply true . apply false . Defined .

Lemma iscontrhfibereqbx ( X : Type ) ( x : X ) ( is : isisolated X x ) : iscontr ( hfiber ( eqbx X x is ) true ) .
...
Definition bhfiber { X Y : Type } ( f : X -> Y ) ( y : Y ) ( is : isisolated Y y ) := hfiber ( fun x : X => eqbx Y y is ( f x ) ) true .

Lemma weqhfibertobhfiber { X Y : Type } ( f : X -> Y ) ( y : Y ) ( is : isisolated Y y ) : weq ( hfiber f y ) ( bhfiber f y is ) .
...
Corollary iscontrhfiberii1x ( X Y : Type ) ( x : X ) : iscontr ( hfiber ( @ii1 X Y ) ( ii1 x ) ) .
Proof. intros . set ( xe1 :=  hfiberpair ( @ii1 _ _ ) x ( idpath ( @ii1 X Y x ) ) ) . apply ( iscontraprop1 ( isinclii1 X Y ( ii1 x ) ) xe1 ) .  Defined .

Corollary neghfiberii1y ( X Y : Type ) ( y : Y ) : neg ( hfiber ( @ii1 X Y ) ( ii2 y ) ) .
...
Theorem isinclii2 (X Y:Type): isincl  (@ii2 X Y).
...
Corollary iscontrhfiberii2y ( X Y : Type ) ( y : Y ) : iscontr ( hfiber ( @ii2 X Y ) ( ii2 y ) ) .
Proof. intros . set ( xe1 :=  hfiberpair ( @ii2 _ _ ) y ( idpath ( @ii2 X Y y ) ) ) . apply ( iscontraprop1 ( isinclii2 X Y ( ii2 y ) ) xe1 ) .  Defined .

Corollary neghfiberii2x ( X Y : Type ) ( x : X ) : neg ( hfiber ( @ii2 X Y ) ( ii1 x ) ) .
Proof. intros . intro ye . induction ye as [ y e ] . apply ( negpathsii2ii1 _ _ e ) .  Defined.




Lemma negintersectii1ii2 { X Y : Type } (z: coprod X Y): hfiber  (@ii1 X Y) z -> hfiber  (@ii2 _ _) z -> empty.
...
Lemma isolatedtoisolatedii1 (X Y:Type)(x:X)(is:isisolated _ x): isisolated ( coprod X Y ) (ii1 x).
...
Lemma isolatedtoisolatedii2 (X Y:Type)(y:Y)(is:isisolated _ y): isisolated ( coprod X Y ) (ii2 y).
...
Theorem weqhfibercoprodf1 { X Y X' Y' : Type } (f: X -> X')(g:Y -> Y')(x':X'): weq (hfiber  f x') (hfiber  (coprodf   f g) (ii1  x')).
...
Theorem weqhfibercoprodf2 { X Y X' Y' : Type } (f: X -> X')(g:Y -> Y')(y':Y'): weq (hfiber  g y') (hfiber  (coprodf   f g) (ii2  y')).
...
Theorem isofhlevelfcoprodf (n:nat) { X Y Z T : Type } (f : X -> Z ) ( g : Y -> T )( is1 : isofhlevelf n  f ) ( is2 : isofhlevelf n  g ) : isofhlevelf n (coprodf f g).
...
Theorem isofhlevelsnsummand1 ( n : nat ) ( X Y : Type ) : isofhlevel ( S n ) ( coprod X Y ) -> isofhlevel ( S n ) X .
Proof. intros n X Y is . apply ( isofhlevelXfromfY ( S n ) ( @ii1 X Y ) ( isofhlevelfsnincl n _ ( isinclii1 _ _ ) ) is ) .  Defined.


Theorem isofhlevelsnsummand2 ( n : nat ) ( X Y : Type ) : isofhlevel ( S n ) ( coprod X Y ) -> isofhlevel ( S n ) Y .
Proof. intros n X Y is . apply ( isofhlevelXfromfY ( S n ) ( @ii2 X Y ) ( isofhlevelfsnincl n _ ( isinclii2 _ _ ) ) is ) .  Defined.


Theorem isofhlevelssncoprod ( n : nat ) ( X Y : Type ) ( isx : isofhlevel ( S ( S n ) ) X ) ( isy : isofhlevel ( S ( S n ) ) Y ) : isofhlevel ( S ( S n ) ) ( coprod X Y ) .
...
Lemma isasetcoprod ( X Y : Type ) ( isx : isaset X ) ( isy : isaset Y ) : isaset ( coprod X Y ) .
...
Lemma coprodofhfiberstohfiber { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( z : Z ) : coprod ( hfiber f z ) ( hfiber g z ) -> hfiber ( sumofmaps f g ) z .
...
Lemma hfibertocoprodofhfibers { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( z : Z ) :  hfiber ( sumofmaps f g ) z ->  coprod ( hfiber f z ) ( hfiber g z ) .
...
Theorem weqhfibersofsumofmaps { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( z : Z ) : weq ( coprod ( hfiber f z ) ( hfiber g z ) ) ( hfiber ( sumofmaps f g ) z ) .
...
Theorem isofhlevelfssnsumofmaps ( n : nat ) { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( isf : isofhlevelf ( S ( S n ) ) f ) ( isg : isofhlevelf ( S ( S n ) ) g ) : isofhlevelf ( S ( S n ) ) ( sumofmaps f g ) .
...
Lemma noil1 { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( noi : forall ( x : X ) ( y : Y ) , neg ( paths ( f x ) ( g y ) ) ) ( z : Z ) : hfiber f z -> hfiber g z -> empty .
...
Lemma weqhfibernoi1  { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( noi : forall ( x : X ) ( y : Y ) , neg ( paths ( f x ) ( g y ) ) ) ( z : Z ) ( xe : hfiber f z ) : weq ( hfiber ( sumofmaps f g ) z ) ( hfiber f z ) .
...
Lemma weqhfibernoi2  { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( noi : forall ( x : X ) ( y : Y ) , neg ( paths ( f x ) ( g y ) ) ) ( z : Z ) ( ye : hfiber g z ) : weq ( hfiber ( sumofmaps f g ) z ) ( hfiber g z ) .
...
Theorem isofhlevelfsumofmapsnoi ( n : nat ) { X Y Z : Type } ( f : X -> Z ) ( g : Y -> Z ) ( isf : isofhlevelf n f ) ( isg : isofhlevelf n g ) ( noi : forall ( x : X ) ( y : Y ) , neg ( paths ( f x ) ( g y ) ) ) : isofhlevelf n ( sumofmaps f g ) .
...
Definition tocompltoii1x (X Y:Type)(x:X): coprod (compl X x) Y -> compl (coprod X Y) (ii1  x).
...
Definition fromcompltoii1x (X Y:Type)(x:X): compl (coprod X Y) (ii1  x) ->  coprod (compl X x) Y.
...
Theorem isweqtocompltoii1x (X Y:Type)(x:X): isweq (tocompltoii1x X Y x).
...
Definition tocompltoii2y (X Y:Type)(y:Y): coprod X (compl Y y) -> compl (coprod X Y) (ii2  y).
...
Definition fromcompltoii2y (X Y:Type)(y:Y): compl (coprod X Y) (ii2  y) ->  coprod X (compl Y y).
...
Theorem isweqtocompltoii2y (X Y:Type)(y:Y): isweq (tocompltoii2y X Y y).
...
Definition tocompltodisjoint (X:Type): X -> compl (coprod X unit) (ii2  tt) := fun x:_ => complpair _ _ (ii1  x) (negpathsii2ii1 x tt).

Definition fromcompltodisjoint (X:Type): compl (coprod X unit) (ii2  tt) -> X.
Proof. intros X X0. induction X0 as [ t x ].  induction t as [ x0 | u ] . assumption.  induction u. apply (fromempty (x (idpath (ii2 tt)))). Defined.


Lemma isweqtocompltodisjoint (X:Type): isweq (tocompltodisjoint X).
...
Definition weqtocompltodisjoint ( X : Type ) := weqpair _ ( isweqtocompltodisjoint X ) .

Corollary isweqfromcompltodisjoint (X:Type): isweq (fromcompltodisjoint X).
...
Definition isdecprop (P:Type) := (P ⨿ ¬P) × isaprop P.

Definition isdecproptoisaprop ( X : Type ) ( is : isdecprop X ) : isaprop X := pr2 is.
Coercion isdecproptoisaprop : isdecprop >-> isaprop .

Lemma isdecpropif ( X : Type ) : isaprop X -> X ⨿ ¬ X -> isdecprop X.
Proof. intros ? i c. exact (c,,i). Defined.

Lemma isdecpropif' ( X : Type ) : isaprop X -> X ⨿ ¬ X -> iscontr (X ⨿ ¬ X) .
...
Lemma isdecproplogeqf { X Y : Type } ( isx : isdecprop X ) ( isy : isaprop Y ) ( lg : X <-> Y ) : isdecprop Y .
Proof . intros. set ( w := weqimplimpl ( pr1 lg ) ( pr2 lg ) isx isy ) . apply ( isdecpropweqf w isx ) . Defined .

Lemma isdecproplogeqb { X Y : Type } ( isx : isaprop X ) ( isy : isdecprop Y ) ( lg : X <-> Y ) : isdecprop X .
...
Lemma isdecproppaths { X : Type } ( is : isdeceq X ) ( x x' : X ) : isdecprop ( paths x x' ) .
Proof. intros . apply ( isdecpropif _ ( isasetifdeceq _ is x x' ) ( is x x' ) ) .  Defined .

Lemma isdeceqif { X : Type } ( is : forall x x' : X , isdecprop ( paths x x' ) ) : isdeceq X .
Proof . intros . intros x x' . apply ( pr1 ( is x x' ) ) . Defined .

Lemma isaninv1 (X:Type): isdecprop X  -> isaninvprop X.
Proof.
...
Theorem isdecpropfibseq1 { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( z : Z ) ( fs : fibseqstr f g z ) : isdecprop X -> isaprop Z -> isdecprop Y .
...
Theorem isdecpropfibseq0 { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( z : Z ) ( fs : fibseqstr f g z ) : isdecprop Y -> isdeceq Z -> isdecprop X .
...
Theorem isdecpropdirprod { X Y : Type } ( isx : isdecprop X ) ( isy : isdecprop Y ) : isdecprop ( dirprod X Y ) .
...
Lemma isdecproppathsfromisolated ( X : Type ) ( x : X ) ( is : isisolated X x ) ( x' : X ) : isdecprop ( paths x x' ) .
Proof. intros . apply isdecpropif . apply isaproppathsfromisolated .   assumption .  apply ( is x' ) .  Defined .

Lemma isdecproppathstoisolated  ( X : Type ) ( x : X ) ( is : isisolated X x ) ( x' : X ) : isdecprop ( paths x' x ) .
...
Definition isdecincl {X Y:Type} (f :X -> Y) := ∀ y:Y, isdecprop ( hfiber f y ).
Lemma isdecincltoisincl { X Y : Type } ( f : X -> Y ) : isdecincl f -> isincl f .
Proof. intros X Y f is . intro y . apply ( isdecproptoisaprop _ ( is y ) ) . Defined.
Coercion isdecincltoisincl : isdecincl >-> isincl .

Lemma isdecinclfromisweq { X Y : Type } ( f : X -> Y ) : isweq f -> isdecincl f .
Proof. intros X Y f iswf .  intro y .  apply ( isdecpropfromiscontr ( iswf y ) ) . Defined .

Lemma isdecpropfromdecincl { X Y : Type } ( f : X -> Y ) : isdecincl f -> isdecprop Y -> isdecprop X .
...
Lemma isdecinclii1 (X Y: Type): isdecincl ( @ii1 X Y ) .
...
Lemma isdecinclii2 (X Y: Type): isdecincl ( @ii2 X Y ) .
...
Lemma isdecinclpr1 { X : Type } ( P : X -> Type ) ( is : forall x : X , isdecprop ( P x ) ) : isdecincl ( @pr1 _ P ) .
Proof . intros . intro x . assert ( w : weq ( P x ) ( hfiber (@pr1 _ P )  x ) ) . apply ezweqpr1 .  apply ( isdecpropweqf w ( is x ) ) . Defined .


Theorem isdecinclhomot { X Y : Type } ( f g : X -> Y ) ( h : forall x : X , paths ( f x ) ( g x ) ) ( is : isdecincl f ) : isdecincl g .
Proof. intros . intro y . apply ( isdecpropweqf ( weqhfibershomot f g h y ) ( is y ) ) . Defined .


Theorem isdecinclcomp { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( isf : isdecincl f ) ( isg : isdecincl g ) : isdecincl ( fun x : X => g ( f x ) ) .
...
Theorem isdecinclf { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( isg : isincl g ) ( isgf : isdecincl ( fun x : X => g ( f x ) ) ) : isdecincl f .
...
Theorem isdecinclg { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( isf : isweq f ) ( isgf : isdecincl ( fun x : X => g ( f x ) ) ) : isdecincl g .
...
Theorem isisolateddecinclf { X Y : Type } ( f : X -> Y ) ( x : X ) : isdecincl f -> isisolated X x -> isisolated Y ( f x ) .
...
Definition negimage { X Y : Type } ( f : X -> Y ) := total2 ( fun y : Y => neg ( hfiber f y ) ) .
Definition negimagepair { X Y : Type } ( f : X -> Y ) := tpair ( fun y : Y => neg ( hfiber f y ) ) .

Lemma isinclfromcoprodwithnegimage { X Y : Type } ( f : X -> Y ) ( is : isincl f ) : isincl ( sumofmaps f ( @pr1 _ ( fun y : Y => neg ( hfiber f y ) ) ) ) .
...
Definition iscoproj { X Y : Type } ( f : X -> Y ) := isweq ( sumofmaps f ( @pr1 _ ( fun y : Y => neg ( hfiber f y ) ) ) ) .

Definition weqcoproj { X Y : Type } ( f : X -> Y ) ( is : iscoproj f ) : weq ( coprod X ( negimage f ) ) Y := weqpair _ is .

Theorem iscoprojfromisdecincl { X Y : Type } ( f : X -> Y ) ( is : isdecincl f ) : iscoproj f .
...
Theorem isdecinclfromiscoproj { X Y : Type } ( f : X -> Y ) ( is : iscoproj f ) : isdecincl f .
...
Definition etacorrection: forall T:Type, forall P:T -> Type, forall f: (forall t:T, P t), paths f (fun t:T => f t).
Proof. reflexivity. Defined.

Lemma isweqetacorrection { T : Type } (P:T -> Type): isweq (fun f: forall t:T, P t => (fun t:T => f t)).
Proof. intros. apply idisweq. Defined.

Definition weqeta { T : Type } (P:T -> Type) := weqpair _ ( isweqetacorrection P ) .

Lemma etacorrectiononpaths { T : Type } (P:T->Type)(s1 s2 :forall t:T, P t) : paths (fun t:T => s1 t) (fun t:T => s2 t)-> paths s1 s2.
Proof. intros T P s1 s2 X. set (ew := weqeta P). apply (invmaponpathsweq ew s1 s2 X). Defined.

Definition etacor { X Y : Type } (f:X -> Y) : paths f (fun x:X => f x) := etacorrection _ (fun T:X => Y) f.

Lemma etacoronpaths { X Y : Type } (f1 f2 : X->Y) : paths (fun x:X => f1 x) (fun x:X => f2 x) -> paths f1 f2.
...
Definition toforallpaths { T : Type }  (P:T -> Type) (f g :forall t:T, P t) : (paths f g) -> (forall t:T, paths (f t) (g t)).
Proof. intros T P f g X t. induction X. apply (idpath (f t)). Defined.


Definition sectohfiber { X : Type } (P:X -> Type): (forall x:X, P x) -> (hfiber (fun f:_ => fun x:_ => pr1  (f x)) (fun x:X => x)) := (fun a : forall x:X, P x => tpair _ (fun x:_ => tpair _ x (a x)) (idpath (fun x:X => x))).

Definition hfibertosec  { X : Type } (P:X -> Type):  (hfiber (fun f:_ => fun x:_ => pr1  (f x)) (fun x:X => x)) -> (forall x:X, P x):= fun se:_  => fun x:X => match se as se' return P x with tpair _ s e => (transportf P (toforallpaths (fun x:X => X)  (fun x:X => pr1 (s x)) (fun x:X => x) e x) (pr2  (s x))) end.

Definition sectohfibertosec { X : Type } (P:X -> Type): forall a: forall x:X, P x, paths (hfibertosec _  (sectohfiber _ a)) a := fun a:_ => (pathsinv0 (etacorrection _ _ a)).
