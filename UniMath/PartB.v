...
and only once as a type in the definition of the family isofhlevel : nat -> Type as a fixpoint with
values in Type. *)
...
Theorem hlevelretract (n:nat) { X Y : Type } ( p : X -> Y ) ( s : Y -> X ) ( eps : forall y : Y , paths ( p ( s y ) ) y ) : isofhlevel n X -> isofhlevel n Y .
...
Corollary  isofhlevelweqf (n:nat) { X Y : Type } ( f : weq X Y ) : isofhlevel n X  ->  isofhlevel n Y .
Proof. intros n X Y f X0.  apply (hlevelretract n  f (invmap f ) (homotweqinvweq  f )). assumption. Defined.

Corollary  isofhlevelweqb (n:nat) { X Y : Type } ( f : weq X Y ) : isofhlevel n Y  ->  isofhlevel n X .
Proof. intros n X Y f X0 .  apply (hlevelretract n  (invmap  f ) f (homotinvweqweq  f )). assumption. Defined.

Lemma isofhlevelsn ( n : nat ) { X : Type } ( f : X -> isofhlevel ( S n ) X ) : isofhlevel ( S n ) X.
Proof. intros . simpl . intros x x' . apply ( f x x x'). Defined.

Lemma isofhlevelssn (n:nat) { X : Type } ( is : forall x:X, isofhlevel (S n) (paths x x)) : isofhlevel (S (S n)) X.
...
Definition isofhlevelf ( n : nat ) { X Y : Type } ( f : X -> Y ) : Type := forall y:Y, isofhlevel n (hfiber  f y).


Theorem isofhlevelfhomot ( n : nat ) { X Y : Type }(f f':X -> Y)(h: forall x:X, paths (f x) (f' x)): isofhlevelf n f -> isofhlevelf n  f'.
Proof. intros n X Y f f' h X0. unfold isofhlevelf. intro y . apply ( isofhlevelweqf n ( weqhfibershomot f f' h y ) ( X0 y )) .   Defined .


Theorem isofhlevelfpmap ( n : nat ) { X Y : Type } ( f : X -> Y ) ( Q : Y -> Type ) : isofhlevelf n  f -> isofhlevelf n ( fpmap f Q ) .
...
Theorem isofhlevelfffromZ ( n : nat ) { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( z : Z ) ( fs : fibseqstr f g z ) ( isz : isofhlevel ( S n ) Z ) : isofhlevelf n f .
...
Theorem isofhlevelXfromg ( n : nat ) { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( z : Z ) ( fs : fibseqstr f g z ) : isofhlevelf n g -> isofhlevel n X  .
...
Theorem isofhlevelffromXY ( n : nat ) { X Y : Type } ( f : X -> Y ) : isofhlevel n X -> isofhlevel (S n) Y -> isofhlevelf n f.
...
Theorem isofhlevelXfromfY ( n : nat ) { X Y : Type } ( f : X -> Y ) : isofhlevelf n f -> isofhlevel n Y -> isofhlevel n X.
...
Theorem  isofhlevelffib ( n : nat ) { X : Type } ( P : X -> Type ) ( x : X ) ( is : forall x':X, isofhlevel n (paths x' x) ) : isofhlevelf n ( tpair P x ) .
Proof . intros . unfold isofhlevelf . intro xp .   apply (isofhlevelweqf n ( ezweq1pr1 P x xp) ( is ( pr1 xp ) ) ) . Defined .



Theorem isofhlevelfhfiberpr1y ( n : nat ) { X Y : Type } ( f : X -> Y ) ( y : Y ) ( is : forall y':Y, isofhlevel n (paths  y' y) ) : isofhlevelf n ( hfiberpr1 f y).
...
Theorem isofhlevelfsnfib (n:nat) { X : Type } (P:X -> Type)(x:X) ( is : isofhlevel (S n) (paths x x) ) : isofhlevelf (S n) ( tpair P x ).
...
Theorem isofhlevelfsnhfiberpr1 ( n : nat ) { X Y : Type } (f : X -> Y ) ( y : Y ) ( is : isofhlevel (S n) (paths y y) ) : isofhlevelf (S n) (hfiberpr1 f y).
...
Corollary isofhlevelfhfiberpr1 ( n : nat ) { X Y : Type }  ( f : X -> Y ) ( y : Y ) ( is : isofhlevel (S n) Y ) : isofhlevelf n ( hfiberpr1 f y ) .
...
Theorem isofhlevelff ( n : nat ) { X Y Z : Type } (f : X -> Y ) ( g : Y -> Z ) : isofhlevelf n  (fun x : X => g ( f x) ) -> isofhlevelf (S n)  g -> isofhlevelf n  f.
...
Theorem isofhlevelfgf ( n : nat ) { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) : isofhlevelf n  f -> isofhlevelf n  g -> isofhlevelf n  (fun x:X => g(f x)).
...
Theorem isofhlevelfgwtog (n:nat ) { X Y Z : Type } ( w : weq X Y ) ( g : Y -> Z ) ( is : isofhlevelf n  (fun x : X => g ( w x ) ) ) : isofhlevelf n g  .
...
Theorem isofhlevelfgtogw (n:nat ) { X Y Z : Type } ( w : weq X Y ) ( g : Y -> Z ) ( is : isofhlevelf n g ) :  isofhlevelf n  (fun x : X => g ( w x ) ) .
...
Corollary isofhlevelfhomot2 (n:nat) { X X' Y : Type } (f:X -> Y)(f':X' -> Y)(w : weq X X' )(h:forall x:X, paths (f x) (f' (w x))) : isofhlevelf n  f -> isofhlevelf n  f'.
...
Theorem isofhlevelfonpaths (n:nat) { X Y : Type }(f:X -> Y)(x x':X): isofhlevelf (S n)  f -> isofhlevelf n  (@maponpaths _ _ f x x').
...
Theorem isofhlevelfsn (n:nat) { X Y : Type } (f:X -> Y): (forall x x':X, isofhlevelf n  (@maponpaths _ _ f x x')) -> isofhlevelf (S n)  f.
...
Theorem isofhlevelfssn (n:nat) { X Y : Type } (f:X -> Y): (forall x:X, isofhlevelf (S n)  (@maponpaths _ _ f x x)) -> isofhlevelf (S (S n))  f.
...
Theorem isofhlevelfpr1 (n:nat) { X : Type } (P:X -> Type)(is: forall x:X, isofhlevel n (P x)) : isofhlevelf n  (@pr1 X P).
Proof. intros. unfold isofhlevelf. intro x .  apply (isofhlevelweqf n  ( ezweqpr1  _ x)    (is x)). Defined.

Lemma isweqpr1 { Z : Type } ( P : Z -> Type ) ( is1 : forall z : Z, iscontr ( P z ) ) : isweq ( @pr1 Z P ) .
Proof. intros. unfold isweq.  intro y. set (isy:= is1 y). apply (iscontrweqf ( ezweqpr1 P y)) . assumption. Defined.

Definition weqpr1 { Z : Type } ( P : Z -> Type ) ( is : forall z : Z , iscontr ( P z ) ) : weq ( total2 P ) Z := weqpair _ ( isweqpr1 P is ) .
...
Theorem isofhleveltotal2 ( n : nat ) { X : Type } ( P : X -> Type ) ( is1 : isofhlevel n X )( is2 : forall x:X, isofhlevel n (P x) ) : isofhlevel n (total2 P).
Proof. intros. apply (isofhlevelXfromfY n  (@pr1 _ _ )). apply isofhlevelfpr1. assumption. assumption. Defined.

Corollary isofhleveldirprod ( n : nat ) ( X Y : Type ) ( is1 : isofhlevel n X ) ( is2 : isofhlevel n Y ) : isofhlevel n (dirprod X Y).
...
Definition isPredicate {X} (Y : X->Type) := ∀ x, isaprop (Y x).
...
Theorem hlevelntosn ( n : nat ) ( T : Type )  ( is : isofhlevel n T ) : isofhlevel (S n) T.
...
Corollary isofhlevelcontr (n:nat) { X : Type } ( is : iscontr X ) : isofhlevel n X.
...
Lemma isofhlevelfweq ( n : nat ) { X Y : Type } ( f : weq X Y ) :  isofhlevelf n f .
Proof. intros n X Y f .  unfold isofhlevelf.   intro y . apply ( isofhlevelcontr n ). apply ( pr2 f ). Defined.

Corollary isweqfinfibseq  { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( z : Z ) ( fs : fibseqstr f g z  ) ( isz : iscontr Z ) : isweq f .
Proof. intros . apply ( isofhlevelfffromZ 0 f g z fs ( isapropifcontr isz ) ) .  Defined .

Corollary weqhfibertocontr { X Y : Type } ( f : X -> Y ) ( y : Y ) ( is : iscontr Y ) : weq ( hfiber f y ) X .
Proof. intros . split with ( hfiberpr1 f y ) . apply ( isofhlevelfhfiberpr1 0 f y ( hlevelntosn 0 _ is ) ) . Defined.

Corollary weqhfibertounit ( X : Type ) : weq ( hfiber ( fun x : X => tt ) tt ) X .
Proof.  intro . apply ( weqhfibertocontr _ tt iscontrunit ) . Defined.

Corollary isofhleveltofun ( n : nat ) ( X : Type ) : isofhlevel n X -> isofhlevelf n ( fun x : X => tt ) .
Proof. intros n X is .  intro t . induction t . apply ( isofhlevelweqb n ( weqhfibertounit X ) is ) .  Defined .

Corollary isofhlevelfromfun ( n : nat ) ( X : Type ) : isofhlevelf n ( fun x : X => tt ) ->  isofhlevel n X .
...
Lemma invproofirrelevance ( X : Type ) ( ee : forall x x' : X , paths x x' ) : isaprop X.
...
Lemma isweqimplimpl { X Y : Type } ( f : X -> Y ) ( g : Y -> X ) ( isx : isaprop X ) ( isy : isaprop Y ) : isweq f.
Proof. intros.
...
Definition weqimplimpl { X Y : Type } ( f : X -> Y ) ( g : Y -> X ) ( isx : isaprop X ) ( isy : isaprop Y ) := weqpair _ ( isweqimplimpl f g isx isy ) .

Definition weqiff { X Y : Type } : (X <-> Y) -> isaprop X -> isaprop Y -> X ≃ Y
  := λ f i j, weqpair _ ( isweqimplimpl (pr1 f) (pr2 f) i j).

Definition weq_to_iff { X Y : Type } : X ≃ Y -> (X <-> Y)
...
Theorem isapropifnegtrue { X : Type } ( a : X -> empty ) : isaprop X .
...
Definition incl ( X Y : Type ) := total2 ( fun f : X -> Y => isincl f ) .
Definition inclpair { X Y : Type } ( f : X -> Y ) ( is : isincl f ) : incl X Y := tpair _ f is .
Definition pr1incl ( X Y : Type ) : incl X Y -> ( X -> Y ) := @pr1 _ _ .
Coercion pr1incl : incl >-> Funclass .

Lemma isinclweq ( X Y : Type ) ( f : X -> Y ) : isweq f -> isincl f .
Proof . intros X Y f is . apply ( isofhlevelfweq 1 ( weqpair _ is ) ) .  Defined .
Coercion isinclweq : isweq >-> isincl .

Lemma isofhlevelfsnincl (n:nat) { X Y : Type } (f:X -> Y)(is: isincl  f): isofhlevelf (S n)  f.
Proof. intros. unfold isofhlevelf.  intro y . apply isofhlevelsnprop. apply (is y). Defined.

Definition weqtoincl ( X Y : Type ) : weq X Y -> incl X Y :=  fun w => inclpair ( pr1weq w ) ( pr2 w ) .
Coercion weqtoincl : weq >-> incl .

Lemma isinclcomp { X Y Z : Type } ( f : incl X Y ) ( g : incl Y Z ) : isincl ( funcomp ( pr1 f ) ( pr1 g ) ) .
Proof . intros . apply ( isofhlevelfgf 1 f g ( pr2 f ) ( pr2 g ) ) . Defined .

Definition inclcomp { X Y Z : Type } ( f : incl X Y ) ( g : incl Y Z ) : incl X Z := inclpair ( funcomp ( pr1 f ) ( pr1 g ) ) ( isinclcomp f g ) .

Lemma isincltwooutof3a { X Y Z : Type } ( f : X -> Y ) ( g : Y -> Z ) ( isg : isincl g ) ( isgf : isincl ( funcomp f g ) ) : isincl f .
Proof . intros . apply ( isofhlevelff 1 f g isgf ) .  apply ( isofhlevelfsnincl 1 g isg ) . Defined .

Lemma isinclgwtog { X Y Z : Type } ( w : weq X Y ) ( g : Y -> Z ) ( is : isincl ( funcomp w g ) ) : isincl g .
Proof . intros . apply ( isofhlevelfgwtog 1 w g is ) .  Defined .

Lemma isinclgtogw { X Y Z : Type }  ( w : weq X Y ) ( g : Y -> Z ) ( is : isincl g ) : isincl ( funcomp w g ) .
Proof . intros . apply  ( isofhlevelfgtogw 1 w g is ) . Defined .


Lemma isinclhomot { X Y : Type } ( f g : X -> Y ) ( h : homot f g ) ( isf : isincl f ) : isincl g .
Proof . intros . apply ( isofhlevelfhomot ( S O ) f g h isf ) . Defined .



Definition isofhlevelsninclb (n:nat) { X Y : Type } (f:X -> Y)(is: isincl  f) : isofhlevel (S n) Y -> isofhlevel (S n) X:= isofhlevelXfromfY (S n)  f (isofhlevelfsnincl n  f is).

Definition  isapropinclb { X Y : Type } ( f : X -> Y ) ( isf : isincl f ) : isaprop Y ->  isaprop X := isofhlevelXfromfY 1 _ isf .


Lemma iscontrhfiberofincl { X Y : Type } (f:X -> Y): isincl  f -> (forall x:X, iscontr (hfiber  f (f x))).
...
Definition isInjective { X Y : Type } (f:X -> Y) := ∀ (x x':X), isweq (maponpaths f : x = x' -> f x = f x').

Definition Injectivity { X Y : Type } (f:X -> Y) :
  isInjective f -> ∀ (x x':X), x = x'  ≃  f x = f x'.
Proof. intros ? ? ? i ? ?. exact (weqpair _ (i x x')). Defined.

Lemma isweqonpathsincl { X Y : Type } (f:X -> Y) : isincl f -> isInjective f.
Proof. intros ? ? ? is x x'. apply (isofhlevelfonpaths O  f x x' is). Defined.

Definition weqonpathsincl  { X Y : Type } (f:X -> Y) (is: isincl  f)(x x':X) := weqpair _ ( isweqonpathsincl f is x x' ) .

Definition invmaponpathsincl { X Y : Type } (f:X -> Y) : isincl f -> ∀ x x', f x = f x' -> x = x'.
Proof.
  intros ? ? ? is x x'.
  exact (invmap (weqonpathsincl  f is x x')).
Defined.

Lemma isinclweqonpaths { X Y : Type } (f:X -> Y): isInjective f -> isincl  f.
Proof. intros X Y f X0.  apply (isofhlevelfsn O f X0). Defined.

Definition isinclpr1 { X : Type } (P:X -> Type)(is: forall x:X, isaprop (P x)): isincl  (@pr1 X P):= isofhlevelfpr1 (S O) P is.

Theorem subtypeInjectivity {A : Type} (B : A -> Type) :
...
Corollary subtypeEquality' {A : Type} {B : A -> Type}
...
Definition subtypePairEquality {X} {P:X -> Type} (is: isPredicate P)
           {x y:X} {p:P x} {q:P y} :
  x = y -> (x,,p) = (y,,q).
Proof. intros X P is x y p q e. apply (total2_paths2 e). apply is. Defined.

Definition subtypePairEquality' {X} {P:X -> Type}
           {x y:X} {p:P x} {q:P y} :
...
Theorem samehfibers { X Y Z : Type } (f: X -> Y) (g: Y -> Z) (is1: isincl g) (y:Y) :
...
Lemma isasetifcontr { X : Type } ( is : iscontr X ) : isaset X .
Proof . intros . apply ( isofhlevelcontr 2 is ) . Defined .

Lemma isasetaprop { X : Type } ( is : isaprop X ) : isaset X .
Proof . intros . apply ( isofhlevelsnprop 1 is ) . Defined .

Corollary isaset_total2 {X:Type} (P:X->Type) : isaset X -> (∀ x, isaset (P x)) -> isaset (Σ x, P x).
Proof. intros. apply (isofhleveltotal2 2); assumption. Defined.

Corollary isaset_dirprod {X Y:Type} : isaset X -> isaset Y -> isaset (X × Y).
...
Lemma uip { X : Type } ( is : isaset X ) { x x' : X } ( e e' : x = x' ) : e = e' .
...
Lemma isofhlevelssnset (n:nat) ( X : Type ) ( is : isaset X ) : isofhlevel ( S (S n) ) X.
Proof. intros n X X0. simpl. unfold isaset in X0.   intros x x' . apply isofhlevelsnprop. set ( int := X0 x x'). assumption . Defined.

Lemma isasetifiscontrloops (X:Type): (forall x:X, iscontr (paths x x)) -> isaset X.
...
Lemma iscontrloopsifisaset (X:Type): (isaset X) -> (forall x:X, iscontr (paths x x)).
...
Theorem isasetsubset { X Y : Type } (f: X -> Y) (is1: isaset Y) (is2: isincl  f): isaset X.
...
Theorem isinclfromhfiber { X Y : Type } (f: X -> Y) (is : isaset Y) ( y: Y ) : @isincl (hfiber  f y) X ( @pr1 _ _  ).
...
Theorem isinclbetweensets { X Y : Type } ( f : X -> Y ) ( isx : isaset X ) ( isy : isaset Y ) ( inj : forall x x' : X , ( paths ( f x ) ( f x' ) -> paths x x' ) ) : isincl f .
...
Theorem isinclfromunit { X : Type } ( f : unit -> X ) ( is : isaset X ) : isincl f .
...
Corollary set_bijection_to_weq {X Y:Type} (f:X->Y) : bijective f -> isaset Y -> isweq f.
Proof.
