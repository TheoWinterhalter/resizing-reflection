...
Axiom funextfunax : forall (X Y:Type)(f g:X->Y),  (forall x:X, paths (f x) (g x)) -> (paths f g).


Lemma isweqlcompwithweq { X X' : Type} (w: weq X X') (Y:Type) : isweq (fun a:X'->Y => (fun x:X => a (w x))).
...
Lemma isweqrcompwithweq { Y Y':Type } (w: weq Y Y')(X:Type): isweq (fun a:X->Y => (fun x:X => w (a x))).
...
Theorem funcontr { X : Type } (P:X -> Type) : (forall x:X, iscontr (P x)) -> iscontr (forall x:X, P x).
...
Corollary funcontrtwice { X : Type } (P: X-> X -> Type)(is: forall (x x':X), iscontr (P x x')): iscontr (forall (x x':X), P x x').
Proof. intros.
...
Lemma funextweql1 { T : Type } (P:T -> Type) (g: ∀ t, P t) : ∃! (f:∀ t, P t), ∀ t:T, f t = g t.
Proof.
...
Theorem isweqtoforallpaths { T : Type } (P:T -> Type) (f g: ∀ t:T, P t) :
...
Theorem weqtoforallpaths { T : Type } (P:T -> Type)(f g : forall t:T, P t) :
  (f = g) ≃ (∀ t:T, f t = g t).
Proof. intros. exists (toforallpaths P f g). apply isweqtoforallpaths. Defined.

Definition funextsec {T:Type} (P: T-> Type) (f g : ∀ t:T, P t) : (∀ t, f t = g t) -> f = g
  := invmap (weqtoforallpaths _ f g) .

Definition funextfun { X Y:Type } (f g:X->Y) : (∀ x:X, f x = g x) -> f = g
  := funextsec _ _ _.
...
Theorem isweqfunextsec { T : Type } (P:T -> Type)(f g : forall t:T, P t) : isweq (funextsec P f g).
Proof. intros. apply (isweqinvmap ( weqtoforallpaths _  f g ) ). Defined.

Definition weqfunextsec { T : Type } (P:T -> Type)(f g : forall t:T, P t) : weq  (forall t:T, paths (f t) (g t)) (paths f g) := weqpair _ ( isweqfunextsec P f g ) .
...
(** ** Sections of "double fibration" [(P: T -> Type)(PP: forall t:T, P t -> Type)] and pairs of sections *)



(** *** General case *)

Definition totaltoforall { X : Type } (P : X -> Type ) ( PP : forall x:X, P x -> Type ) : total2 (fun s0: forall x:X, P x => forall x:X, PP x (s0 x)) -> forall x:X, total2 (PP x).
Proof.
  intros X P PP X0 x.
  exists (pr1 X0 x).
  apply (pr2 X0 x).
...
Definition foralltototal  { X : Type } ( P : X -> Type ) ( PP : forall x:X, P x -> Type ):  (forall x:X, total2 (PP x)) -> total2 (fun s0: forall x:X, P x => forall x:X, PP x (s0 x)).
Proof.
  intros X P PP X0.
...
Lemma lemmaeta1 { X : Type } (P:X->Type) (Q:(forall x:X, P x) -> Type)(s0: forall x:X, P x)(q: Q (fun x:X => (s0 x))): paths (tpair (fun s: (forall x:X, P x) => Q (fun x:X => (s x))) s0 q) (tpair (fun s: (forall x:X, P x) => Q (fun x:X => (s x))) (fun x:X => (s0 x)) q).
...
Definition totaltoforalltototal { X : Type } ( P : X -> Type ) ( PP : forall x:X, P x -> Type )( ss : total2 (fun s0: forall x:X, P x => forall x:X, PP x (s0 x)) ): paths (foralltototal _ _ (totaltoforall  _ _ ss)) ss.
...
Definition foralltototaltoforall { X : Type } ( P : X -> Type ) ( PP : forall x:X, P x -> Type ) ( ss : forall x:X, total2 (PP x)): paths (totaltoforall _ _ (foralltototal _ _ ss)) ss.
...
Theorem isweqforalltototal { X : Type } ( P : X -> Type ) ( PP : forall x:X, P x -> Type ) : isweq (foralltototal P PP).
...
Theorem isweqtotaltoforall { X : Type } (P:X->Type)(PP:forall x:X, P x -> Type): isweq (totaltoforall P PP).
...
Definition weqforalltototal { X : Type } ( P : X -> Type ) ( PP : forall x:X, P x -> Type ) := weqpair _ ( isweqforalltototal P PP ) .

Definition weqtotaltoforall { X : Type } ( P : X -> Type ) ( PP : forall x:X, P x -> Type ) := invweq ( weqforalltototal P PP ) .



(** *** Functions to a dependent sum (to a [ total2 ]) *)

Definition weqfuntototaltototal ( X : Type ) { Y : Type } ( Q : Y -> Type ) : weq ( X -> total2 Q ) ( total2 ( fun f : X -> Y => forall x : X , Q ( f x ) ) ) := weqforalltototal ( fun x : X => Y ) ( fun x : X => Q ) .
...
Definition funtoprodtoprod { X Y Z : Type } ( f : X -> dirprod Y Z ) : dirprod ( X -> Y ) ( X -> Z ) := dirprodpair ( fun x : X => pr1 ( f x ) ) ( fun x : X => ( pr2 ( f x ) ) ) .

Definition prodtofuntoprod { X Y Z : Type } ( fg : dirprod ( X -> Y ) ( X -> Z ) ) : X -> dirprod Y Z := match fg with tpair _ f g => fun x : X => dirprodpair ( f x ) ( g x ) end .

Theorem weqfuntoprodtoprod ( X Y Z : Type ) : weq ( X -> dirprod Y Z ) ( dirprod ( X -> Y ) ( X -> Z ) ) .
...
Definition maponsec { X:Type }  (P Q : X -> Type) (f: forall x:X, P x -> Q x): (forall x:X, P x) -> (forall x:X, Q x) :=
fun s: forall x:X, P x => (fun x:X => (f x) (s x)).

Definition maponsec1 { X Y : Type } (P:Y -> Type)(f:X-> Y): (forall y:Y, P y) -> (forall x:X, P (f x)) := fun sy: forall y:Y, P y => (fun x:X => sy (f x)).



Definition hfibertoforall { X : Type } (P Q : X -> Type) (f: forall x:X, P x -> Q x)(s: forall x:X, Q x): hfiber  (@maponsec _ _ _ f) s -> forall x:X, hfiber  (f x) (s x).
...
Definition foralltohfiber  { X : Type } ( P Q : X -> Type) (f: forall x:X, P x -> Q x)(s: forall x:X, Q x): (forall x:X, hfiber  (f x) (s x)) -> hfiber  (maponsec _ _ f) s.
Proof.  intro. intro. intro. intro.   intro. unfold hfiber.
...
Theorem isweqhfibertoforall  { X : Type } (P Q :X -> Type) (f: forall x:X, P x -> Q x)(s: forall x:X, Q x): isweq (hfibertoforall _ _ f s).
...
Definition weqhfibertoforall  { X : Type } (P Q :X -> Type) (f: forall x:X, P x -> Q x)(s: forall x:X, Q x) := weqpair _ ( isweqhfibertoforall P Q f s ) .



Theorem isweqforalltohfiber  { X : Type } (P Q : X -> Type) (f: forall x:X, P x -> Q x)(s: forall x:X, Q x): isweq (foralltohfiber  _ _ f s).
Proof. intro. intro. intro. intro. intro.
...
Definition weqforalltohfiber  { X : Type } (P Q : X -> Type) (f: forall x:X, P x -> Q x)(s: forall x:X, Q x) := weqpair _ ( isweqforalltohfiber P Q f s ) .
...
Corollary isweqmaponsec { X : Type } (P Q : X-> Type) (f: forall x:X, weq ( P x ) ( Q x) ) : isweq (maponsec _ _ f).
...
Definition weqonsecfibers { X : Type } (P Q : X-> Type) (f: forall x:X, weq ( P x ) ( Q x )) := weqpair _ ( isweqmaponsec P Q f ) .


(** *** Composition of functions with a weak equivalence on the right *)

Definition  weqffun ( X : Type ) { Y Z : Type } ( w : weq Y Z ) : weq ( X -> Y ) ( X -> Z ) := weqonsecfibers _ _ ( fun x : X => w ) .
...
Definition maponsec1l0 { X : Type } (P:X -> Type)(f:X-> X)(h: forall x:X, paths (f x) x)(s: forall x:X, P x): (forall x:X, P x) := (fun x:X => transportf P  (h x) (s (f x))).

Lemma maponsec1l1  { X : Type } (P:X -> Type)(x:X)(s:forall x:X, P x): paths (maponsec1l0 P (fun x:X => x) (fun x:X => idpath x) s x) (s x).
Proof. intros. unfold maponsec1l0.   apply idpath. Defined.


Lemma maponsec1l2 { X : Type } (P:X -> Type)(f:X-> X)(h: forall x:X, paths (f x) x)(s: forall x:X, P x)(x:X): paths (maponsec1l0 P f h s x) (s x).
Proof. intros.
...
Theorem isweqmaponsec1 { X Y : Type } (P:Y -> Type)(f: weq X Y ) : isweq (maponsec1 P f).
Proof. intros.
...
Definition weqonsecbase { X Y : Type } ( P : Y -> Type ) ( f : weq X Y )
...
Definition  weqbfun  { X Y : Type } ( Z : Type ) ( w : weq X Y ) : weq ( Y -> Z ) ( X -> Z ) := weqonsecbase _ w .
...
Definition iscontrsecoverempty ( P : empty -> Type ) : iscontr ( forall x : empty , P x ) .
Proof . intro . split with ( fun x : empty => fromempty x )  .  intro t .  apply funextsec .  intro t0 . induction t0 . Defined .

Definition iscontrsecoverempty2 { X : Type } ( P : X -> Type ) ( is : neg X ) : iscontr ( forall x : X , P x ) .
Proof . intros .  set ( w := weqtoempty is ) . set ( w' := weqonsecbase P ( invweq w ) ) .   apply ( iscontrweqb w' ( iscontrsecoverempty _ ) ) . Defined .

Definition secovercoprodtoprod { X Y : Type } ( P : coprod X Y -> Type ) ( a: forall xy : coprod X Y , P xy ) : dirprod ( forall x : X , P ( ii1 x ) ) ( forall y : Y , P ( ii2 y ) ) := dirprodpair ( fun x : X => a ( ii1 x ) ) ( fun y : Y => a ( ii2 y ) )  .

Definition prodtosecovercoprod { X Y : Type } ( P : coprod X Y -> Type ) ( a : dirprod ( forall x : X , P ( ii1 x ) ) ( forall y : Y , P ( ii2 y ) ) ) :  forall xy : coprod X Y , P xy .
Proof . intros . induction xy as [ x | y ] . apply ( pr1 a x ) .    apply ( pr2 a y ) . Defined .


Definition weqsecovercoprodtoprod { X Y : Type } ( P : coprod X Y -> Type ) : weq ( forall xy : coprod X Y , P xy ) ( dirprod ( forall x : X , P ( ii1 x ) ) ( forall y : Y , P ( ii2 y ) ) ) .
...
Theorem iscontrfunfromempty ( X : Type ) : iscontr ( empty -> X ) .
Proof . intro . split with fromempty . intro t .  apply funextfun .  intro x . induction x . Defined .

Theorem iscontrfunfromempty2 ( X : Type ) { Y : Type } ( is : neg Y ) : iscontr ( Y -> X ) .
...
Definition funfromcoprodtoprod { X Y Z : Type } ( f : coprod X Y -> Z ) : dirprod ( X -> Z ) ( Y -> Z ) := dirprodpair ( fun x : X => f ( ii1 x ) ) ( fun y : Y => f ( ii2 y ) ) .

Definition prodtofunfromcoprod { X Y Z : Type } ( fg : dirprod ( X -> Z ) ( Y -> Z ) ) : coprod X Y -> Z := match fg with tpair _ f g => sumofmaps f g end .

Theorem weqfunfromcoprodtoprod ( X Y Z : Type ) : weq ( coprod X Y -> Z ) ( dirprod ( X -> Z ) ( Y -> Z ) ) .
...
Definition tosecoverunit ( P : unit -> Type ) ( p : P tt ) : forall t : unit , P t .
Proof . intros . induction t . apply p . Defined .

Definition weqsecoverunit ( P : unit -> Type ) : weq ( forall t : unit , P t ) ( P tt ) .
...
Definition weqsecovercontr { X : Type } ( P : X -> Type ) ( is : iscontr X ) : weq ( forall x : X , P x ) ( P ( pr1 is ) ) .
Proof . intros . set ( w1 := weqonsecbase P ( wequnittocontr is ) ) . apply ( weqcomp w1 ( weqsecoverunit _ ) ) .  Defined .

Definition tosecovertotal2 { X : Type } ( P : X -> Type ) ( Q : total2 P -> Type ) ( a : forall x : X , forall p : P x , Q ( tpair _ x p ) ) : forall xp : total2 P , Q xp .
Proof . intros . induction xp as [ x p ] . apply ( a x p ) . Defined .


Definition weqsecovertotal2 { X : Type } ( P : X -> Type ) ( Q : total2 P -> Type ) : weq ( forall xp : total2 P , Q xp ) ( forall x : X , forall p : P x , Q ( tpair _ x p ) ) .
...
Definition weqfunfromunit ( X : Type ) : weq ( unit -> X ) X := weqsecoverunit _ .

Definition  weqfunfromcontr { X : Type } ( Y : Type ) ( is : iscontr X ) : weq ( X -> Y ) Y := weqsecovercontr _ is .


(** *** Functions from [ total2 ] *)

Definition weqfunfromtotal2 { X : Type } ( P : X -> Type ) ( Y : Type ) : weq ( total2 P -> Y ) ( forall x : X , P x -> Y ) := weqsecovertotal2 P _ .

(** *** Functions from direct product *)

Definition weqfunfromdirprod ( X X' Y : Type ) : weq ( dirprod X X' -> Y ) ( forall x : X , X' -> Y ) := weqsecovertotal2 _ _ .
...
Theorem impred (n:nat) { T : Type } (P:T -> Type): (forall t:T, isofhlevel n (P t)) -> (isofhlevel n (forall t:T, P t)).
...
Corollary impred_iscontr { T : Type } (P:T -> Type): (forall t:T, iscontr (P t)) -> (iscontr (forall t:T, P t)).
Proof. intros. apply (impred 0). assumption. Defined.

Corollary impred_isaprop { T : Type } (P:T -> Type): (forall t:T, isaprop (P t)) -> (isaprop (forall t:T, P t)).
Proof. apply impred. Defined.

Corollary impred_isaset { T : Type } (P:T -> Type): (forall t:T, isaset (P t)) -> (isaset (forall t:T, P t)).
Proof. intros. apply (impred 2). assumption. Defined.

Corollary impredtwice  (n:nat) { T T' : Type } (P:T -> T' -> Type): (forall (t:T)(t':T'), isofhlevel n (P t t')) -> (isofhlevel n (forall (t:T)(t':T'), P t t')).
...
Corollary impredfun (n:nat)(X Y:Type)(is: isofhlevel n Y) : isofhlevel n (X -> Y).
Proof. intros. apply (impred n (fun x:_ => Y) (fun x:X => is)). Defined.


Theorem impredtech1 (n:nat)(X Y: Type) : (X -> isofhlevel n Y) -> isofhlevel n (X -> Y).
...
Theorem iscontrfuntounit ( X : Type ) : iscontr ( X -> unit ) .
Proof . intro . split with ( fun x : X => tt ) . intro f .   apply funextfun . intro x . induction ( f x ) .  apply idpath . Defined .

Theorem iscontrfuntocontr ( X : Type ) { Y : Type } ( is : iscontr Y ) : iscontr ( X -> Y ) .
...
Lemma isapropimpl ( X Y : Type ) ( isy : isaprop Y ) : isaprop ( X -> Y ) .
...
Theorem isapropneg2 ( X : Type ) { Y : Type } ( is : neg Y ) : isaprop ( X -> Y ) .
...
Theorem iscontriscontr { X : Type } ( is : iscontr X ) : iscontr ( iscontr X ).
Proof. intros X X0 .
...
assert (is1:
     (forall x:X, isaprop (forall  x' : X,
      (fix isofhlevel (n0 : nat) (X1 : Type) {struct n0} : Type :=
         match n0 with
         | O => iscontr X1
...
Corollary isapropisaprop (X:Type) : isaprop (isaprop X).
Proof. intro. apply (isapropisofhlevel (S O)). Defined.

Definition isapropisdecprop ( X : Type ) : isaprop ( isdecprop X ).
Proof.
  intros.
  unfold isdecprop.
...
Corollary isapropisaset (X:Type): isaprop (isaset X).
Proof. intro. apply (isapropisofhlevel (S (S O))). Defined.


Theorem isapropisofhlevelf ( n : nat ) { X Y : Type } ( f : X -> Y ) : isaprop ( isofhlevelf n f ) .
Proof . intros . unfold isofhlevelf .    apply impred . intro y . apply isapropisofhlevel .  Defined .

Definition isapropisincl { X Y : Type } ( f : X -> Y ) := isapropisofhlevelf 1 f .

Lemma isaprop_isInjective { X Y : Type } (f:X -> Y): isaprop (isInjective f).
Proof.
  intros.
  unfold isInjective.
  apply impred; intro.
...
Lemma incl_injectivity { X Y : Type } (f:X -> Y): isincl f ≃ isInjective f.
Proof.
  intros.
  apply weqimplimpl.
...
Theorem isinclpr1weq ( X Y : Type ) : isincl ( pr1weq : X≃Y -> X->Y ) .
Proof. intros . refine (isinclpr1 _ _) . intro f.   apply isapropisweq .  Defined .

Corollary isinjpr1weq ( X Y : Type ) : isInjective ( pr1weq : X≃Y -> X->Y ) .
Proof. intros. apply isweqonpathsincl. apply isinclpr1weq. Defined.

Theorem isinclpr1isolated ( T : Type ) : isincl ( pr1isolated T ) .
Proof . intro . apply ( isinclpr1 _ ( fun t : T => isapropisisolated T t ) ) . Defined .

(** associativity of weqcomp **)

Definition weqcomp_assoc {W X Y Z : Type} (f:W≃X) (g:X≃Y) (h:Y≃Z) : (h∘(g∘f) = (h∘g)∘f) %weq.
...
Theorem weqfweq ( X : Type ) { Y Z : Type } ( w : weq Y Z ) : weq ( weq X Y ) ( weq X Z ) .
...
Theorem weqbweq  { X Y : Type } ( Z : Type ) ( w : weq X Y ) : weq ( weq Y Z ) ( weq X Z ) .
...
Theorem weqweq {X Y:Type} (w:X≃Y) : (X≃X) ≃ (Y≃Y).
Proof.
...
Theorem weqinvweq ( X Y : Type ) : weq ( weq X Y ) ( weq Y X ) .
...
Theorem isofhlevelsnweqtohlevelsn ( n : nat ) ( X Y : Type ) ( is : isofhlevel ( S n ) Y ) : isofhlevel ( S n ) ( weq X Y ) .
Proof . intros .  apply ( isofhlevelsninclb n _ ( isinclpr1weq _ _ ) ) .  apply impred . intro .  apply is .  Defined .

Theorem isofhlevelsnweqfromhlevelsn ( n : nat ) ( X Y : Type ) ( is : isofhlevel ( S n ) Y ) : isofhlevel ( S n ) ( weq Y X ) .
...
Theorem isapropweqtocontr ( X : Type ) { Y : Type } ( is : iscontr Y ) : isaprop ( weq X Y ) .
Proof . intros .  apply ( isofhlevelsnweqtohlevelsn 0 _ _ ( isapropifcontr is ) ) . Defined .

Theorem isapropweqfromcontr ( X : Type ) { Y : Type } ( is : iscontr Y ) : isaprop ( weq Y X ) .
...
Theorem isapropweqtoprop ( X  Y : Type ) ( is : isaprop Y ) : isaprop ( weq X Y ) .
Proof . intros .  apply ( isofhlevelsnweqtohlevelsn 0 _ _ is ) . Defined .

Theorem isapropweqfromprop ( X Y : Type )( is : isaprop Y ) : isaprop ( weq Y X ) .
Proof. intros .  apply ( isofhlevelsnweqfromhlevelsn 0 X _ is ) . Defined .


(** *** Weak equivalences to and from sets *)

Theorem isasetweqtoset ( X  Y : Type ) ( is : isaset Y ) : isaset ( weq X Y ) .
Proof . intros .  apply ( isofhlevelsnweqtohlevelsn 1 _ _ is ) . Defined .

Theorem isasetweqfromset ( X Y : Type )( is : isaset Y ) : isaset ( weq Y X ) .
...
Definition cutonweq {T:Type} t (is:isisolated T t) (w : T ≃ T ) :
...
Definition weqcutonweq (T:Type) (t:T) (is:isisolated T t) : (T ≃ T) ≃ isolated T × (compl T t ≃ compl T t) .
Proof.
...
Definition locsplit (X:Type)(Y:Type)(f:X -> Y):= forall y:Y, coprod (hfiber  f y) (hfiber  f y -> empty).

Definition dnegimage (X:Type)(Y:Type)(f:X -> Y):= total2 Y (fun y:Y => dneg(hfiber  f y)).
Definition dnegimageincl (X Y:Type)(f:X -> Y):= pr1 Y (fun y:Y => dneg(hfiber  f y)).

Definition xtodnegimage (X:Type)(Y:Type)(f:X -> Y): X -> dnegimage  f:= fun x:X => tpair  (f x) ((todneg _) (hfiberpair  f (f x) x (idpath (f x)))).

Definition locsplitsec (X:Type)(Y:Type)(f:X->Y)(ls: locsplit  f): dnegimage  f -> X := fun u: _ =>
match u with
tpair y psi =>
match (ls y) with
ii1 z => pr1  z|
...
Definition locsplitsecissec  (X Y:Type)(f:X->Y)(ls: locsplit  f)(u:dnegimage  f): paths (xtodnegimage  f (locsplitsec  f ls u)) u.
...
Definition negimage (X:Type)(Y:Type)(f:X -> Y):= total2 Y (fun y:Y => neg(hfiber  f y)).
Definition negimageincl (X Y:Type)(f:X -> Y):= pr1 Y (fun y:Y => neg(hfiber  f y)).


Definition imsum (X:Type)(Y:Type)(f:X -> Y): coprod (dnegimage  f) (negimage  f) -> Y:= fun u:_ =>
match u with
ii1 z => pr1  z|
ii2 z => pr1  z
end.
