...
\begin{document}
  ...
  \begin{lemma}[Validity of $\beta$-reduction]
    ...
    \end{mathc}
  \end{lemma}
  I believe this is simpler than before as we basically only deal with identity
  proofs.
  ...
  identity. We only treat the case of squash for now.}
  ...
  \begin{proof}
    ...
    for some $q,r$.
    We then get
    $\Delta \der \hideq{\app q\ r} f \sigma\ t \sigma \hEr
    (\ap (\lambda x.T \sigma)\ (r\inv)_*) f \sigma'\ t \sigma' : T \sigma$
    (assuming our system to be normalizing).
  ...
  \begin{proof}
    ...
    \begin{caselist}
      ...
      We consider $f \sigma\ u$ and $f' \sigma'\ u'$, they have
      respective whnfs $b$ and $b'$, both of type $A$,
      thus $h \sigma$ (by instantiation of the hypothesis) is a proof
      of their equality, so
      $\Gamma \der \hideq{h \sigma\ b\ b'} b \Er b' : A$ holds.
      This concludes our goal.
  ...
  \begin{proof}
    By induction on the derivation (and by using $\refl$ in the third case).
  \end{proof}
