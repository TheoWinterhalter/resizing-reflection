...
\begin{document}
  ...
  \begin{lemma}[Function type injectivity is valid]
    ...
    $\Gamma, x:U \Der T = T'$. \meta{We need a substitution for $T'$!}
  \end{lemma}

  \begin{proof}
    Assume arbitrary $\Delta \der \sigma : \Gamma$.
    We have
    $\Delta \der \Pi x:U \sigma. T\sigma \hEr \Pi x:U'\sigma. T' \sigma : s_3$
    for some $s_3$. Thus, by definition,
    $\Delta \der \hideq{p} U \sigma \hEr U' \sigma : s_1$ for some $s_1$ and
    $p$. So, $\Gamma \Der U = U'$.

    We also have
    $\Delta, x:U\sigma \der T \sigma \hEr
    \lambda (x:U\sigma). T'\sigma[p_*\ x/x] : s_2$
    for some $s_2$.
