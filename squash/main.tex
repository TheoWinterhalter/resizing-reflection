...
\begin{document}
  ...
  \begin{mathc}
    \dru{\Gamma \der e : \Id (\Pi x:U.T)\ f\ f'
  ...
  \(
    ...
    \ru{\Delta \der \sigma \hEr \sigma' : \Gamma \qquad
        \Delta \der \hideq{\refl} \sigma(x) \hEr \sigma'(x) : U \sigma
  ...
  \begin{proof}
    It is implied by transitivity and symmetry for terms.
  \end{proof}
  ...
  \begin{proof}
    ...
    (\ap (\lambda x.T)\ p\inv)_* ((\lambda x:U\sigma. t\sigma\ x)\ u') :
    \Pi x:U\sigma.T\sigma$.

    This reduces to proving
    $\Delta' \der \hideq{\app \refl\ p} t\sigma\ u \hEr
    (\ap (\lambda x.T)\ p\inv)_* (t \sigma\ u') :
  ...
  \begin{proof}
    Assume $\Delta \der \sigma : \Gamma$ and show
    ...
    \Pi x:U\sigma.V\sigma$.
    \meta{This doesn't work as it is because we need to have different
    substitutions on the left and the right (even though convertible).}
  \end{proof}
