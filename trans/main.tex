...
\begin{document}
  ...
  \begin{proof}
    ...
    \begin{caselist}
      ...
      The problem is that $\lambda (x : (e_1)_*\ A'_2). (e_3)_*\ b'_2$ has type
      $\Pi (x : (e_1)_*\ A'_2). (e_2)_*\ B''$ instead of
      $\Pi (x : (e_1)_*\ A'_1). (e_2)_*\ B''$ (\meta{It's acutally not even
      well-typed as $b'_2$ mentions a $x$ in $(e_1)_*\ A'_1$! But wouldn't
      a substitution $[x := ((e_1)_* (h_1\inv))_*\ x]$ corrupt the translation
      property? -- It seems to work.})
      However these two types are equal thanks to $h_1$.
      \meta{We could explicit it, but everyone should be convinced. We can then
      transport the whole $\lambda$ expression, we keep the translation
      property and thus equality by destructing $e$}.

      We finally have
      \[
        \Gamma' \dere ?? : \Id (\Pi (x : (e_1)_*\ A'_1). (e_2)_*\ B'')\ %
        (\lambda (x : (e_1)_*\ A'_1). (e_3)_*\ b'_1)\ ??.
      \]
    \end{caselist}
  \end{proof}
