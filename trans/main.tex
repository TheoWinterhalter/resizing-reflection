...
\begin{document}
  ...
  (this one will be called $\lambda_r$).
  ...
  Finally, we define $\lambda_b$ which is an intermediary for the translation
  with residuals for $\mR$ and $\me$ : $\mbB$ and $\mbe$.
  This is simply an extension of $\lambda_e$ but where
  $\mbe : \Id s\ \mA\ \mbB$ and $\mbB$ behaves as $\mB$ but can be abstracted
  over, meaning we can destruct $\mbe$.
  ...
  \begin{proof}
    ...
    \begin{caselist}
      ...
      \begin{graycase}
        \begin{mathc}
          \tru{\Gamma \derr A_1 = A_2 : s \qquad
               \Gamma \derr C_1 = C_2 : \Pi x:A_1. \Pi y:A_1. (\Id A_1\ x\ y) \to s'
