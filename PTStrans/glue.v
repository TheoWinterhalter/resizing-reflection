...
Lemma weak_type_shape : forall Γ M N A, Γ ⊢ M ▹ N : A -> forall P B, Γ ⊢ M ▹ P : B ->
 ...
 (exists U u v, Γ ⊢ M ▹ Id U u v : A /\ Γ ⊢ M ▹ Id U u v : B) \/
 (exists U u, Γ ⊢ M ▹ refl U u : A /\ Γ ⊢ M ▹ refl U u : B) \/
 (exists U C b u v p t, Γ ⊢ M ▹ J t U C b u v p : A /\ Γ ⊢ M ▹ J t U C b u v p : B).
  induction 1; intros.
  ...
  - clear IHtyp1 IHtyp2. apply pgen_pi in H2 as (A'' & B'' & s & t & u & h); decompose [and] h; clear h.
    right; right; right. left. exists A'; exists B'; split. eapply typ_pi; eauto.
  ...
  - right. right. right. right. left.
    exists A, u, v. split.
    + apply typ_id ; admit. (* left reflexivity *)
    + admit.
  - right. right. right. right. right. left.
    exists A, u. split ; admit.
  - admit.
  (**)
  ...
  - destruct (IHtyp1 P B0 H1) as [ | [ | [ | ] ] ]. left; eauto.
    ...
    (* destruct H2 as (U & V & ? & ?). right; right; right. exists U; exists V; split. eapply typ_pcompat with A; eauto. trivial. *) admit.
  (**)
  - destruct (IHtyp1 P B0 H1) as [ | [ | [ | ] ] ]. left; eauto.
    ...
    (* destruct H2 as (U & V & ? & ?). right; right; right. exists U; exists V; split. eapply typ_pcompat with B; eauto. trivial. *) admit.
Admitted.
