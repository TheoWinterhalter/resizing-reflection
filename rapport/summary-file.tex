...
Agda~\cite{norell2007towards}, in order to avoid paradoxes \emph{à la} Russell,
...
can alleviate these restrictions in some specific cases.
...
Our proof relies on some parametricity hypotheses commonly believed to
...
report. We will write \cqf{name} to reference the function called \verb|name|
in the Coq file.
...
\ie it is not provable that $\forall x,y : \S1.\ x = y$. It is in a sense
...
of transport.
Basically, given $p : A =_{\Type} B$, it is possible~\cqf{ex\_transport} to
...
\noindent
It has a precise definition~\cqf{ap\_f} that we won't state here, but it
...
transitivity~\cqf{ex\_sym,ex\_trans}.
...
Using transports it is trivial~\cqf{ex\_eq\_equiv} to construct an equivalence
...
$\even := \{ n : \mathbb{N} \mid \squash{\exists p.\ n = 2 \times p} \}$ the
subset of even natural numbers (herein, $\squash{T}$ turns $T$ into a mere
proposition by identifying all of its elements).
...
$B$ was originally intended as a subset of $A$.
...
in most cases) and $A$ in some way. Mike Shulman~\cite{ShulmanResize} for
instance proposed to say that $\RR\ A$ and $A$ are equivalent, but in this
precise case, this is not really interesting as we could have used $B$ from the
start.
...
decidability.
%
This can be achived in Coq thanks to Pierre-Marie Pédrot who, after a discussion
with us, proposed a pull-request~\cite{pperdot:pr} that deactivates the universe
checking locally to create a record that holds a value of type $A$.
Note that this will be available in Coq~8.6 as the pull-request was accepted.
...
in $\top$~\cqf{prop\_in\_top} and that $A \approx B$ implies $A \embd B$.
...
assumed (it holds in the simplicial model~\cite{kapulkin2012simplicial}).
...
That way $T' : \Type_0$ and $T \approx T'$~\cqf{equivHProp}.
...
univalence~\cqf{equiv\_hprop\_bool}.
...
refer to the Coq file for more information~\cqf{embed\_image}.
...
Constructions (hereafter called \CCe) into the regular Calculus of Constructions
...
All judgements in {\CCe} will be written with $\dere$ whereas CC will use
$\der$.
\begin{mathc}
  \nmru{Ext%
...
All of this can be found in the Coq file~\cqf{typeq,HSubst,HProd,HLam,HApp}.
...
\begin{proof}
  By theorem~\ref{thm:valid}.1 we have the translation and by
